...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_scd2_dbdt_dbdt_bed</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data from delta table to delta table</td></tr>
# MAGIC   <tr><td><b>Author</b></td><td>Karthik</td></tr>
# MAGIC   <tr><td><b>Creation Date</b></td><td>2022/06/20</td></tr>
# MAGIC </table>
# MAGIC 
# MAGIC <table>
# MAGIC   <tr>
...
try:
  import pandas as pd
  import pyodbc
  from pyspark.sql.types import StructType,StructField,StringType,IntegerType,DecimalType,ShortType,TimestampType
...
# DBTITLE 1,Add BatchEffectiveDate Filter
try:
    bedFilter = 'AND {batch_effective_datetime} >= LakeValidFromTimestamp AND {batch_effective_datetime} < COALESCE(LakeValidToTimestamp, CURRENT_TIMESTAMP())'.format(batch_effective_datetime = "'" + str(batchEffectiveDatetime) + "'")
    whereExpression = whereExpression+bedFilter
    logTaskProgress(cursor,batchTaskId,"Bed filter added to where clause")
except Exception as e:
    errorMessage="Error while adding bed filter to where clause: " + str(e)
    logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
    assert False  
...
# DBTITLE 1,Register Hash Function
try:
    computeHashValueUdfRegistration()
    calculatelakeDeletedTimestampUdfRegistration()
...
try: 
    applySCDType2onDestForBatchLoad(sourceSelectWhereDF,targetObjectName,columnList,batchEffectiveDatetime,currentTs,cursor,batchTaskId,batchId,adfPipelineName,clusterId,notebookName,errorLogFileLocation) 
