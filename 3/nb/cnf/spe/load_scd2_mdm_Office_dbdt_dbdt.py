...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_mdm_Office_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Office table in conformed zone</td></tr>
...
SELECT 
    ...
    lakelastupdatetimestamp,
    lakeDeletedTimestamp,
    ROW_NUMBER() OVER(PARTITION BY Office_Code,Office_Name ORDER BY Office_Code, Office_Name) AS rnk
FROM (
      -- to fetch the office code and office name from Fin104 mdm table
          SELECT
          Fin104_Office_Code                AS Office_Code, 
          upper(trim(Fin104_Office_Name))   AS Office_Name,
          lakelastupdatedate,
          lakelastupdatetimestamp,
          CASE 
              WHEN lakeDeletedTimestamp IS NOT NULL THEN {batch_effective_datetime} ELSE NULL 
          END                               AS lakeDeletedTimestamp
          FROM standardised_mdm.mdm_fin104_office
      ...
      -- to fetch the office code and office name from Claim_Office mdm table
          SELECT
          Claim_Office_Code                 AS Office_Code, 
          upper(trim(Claim_Office_Name))    AS Office_Name,
          lakelastupdatedate,
          lakelastupdatetimestamp,
          CASE 
              WHEN lakeDeletedTimestamp IS NOT NULL THEN {batch_effective_datetime} ELSE NULL 
          END                               AS lakeDeletedTimestamp
          FROM standardised_mdm.mdm_claim_office
          WHERE {batch_effective_datetime} >= lakevalidfromtimestamp
          AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
          
      UNION 
      
      -- to fetch the office code and office name from Fronting mdm table
          SELECT
          Fronting_Office_Code              AS Office_Code, 
          upper(trim(Fronting_Office_Name)) AS Office_Name,
          lakelastupdatedate,
          lakelastupdatetimestamp,
          CASE 
              WHEN lakeDeletedTimestamp IS NOT NULL THEN {batch_effective_datetime} ELSE NULL 
          END                               AS lakeDeletedTimestamp
          FROM standardised_mdm.mdm_fronting_office
          WHERE {batch_effective_datetime} >= lakevalidfromtimestamp
          AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
          
      UNION 
      
      -- to fetch the office code and office name from Underwriting mdm table
          SELECT 
          Underwriting_Office_Code              AS Office_Code, 
          upper(trim(Underwriting_Office_Name)) AS Office_Name,
          lakelastupdatedate,
          lakelastupdatetimestamp,
          CASE 
              WHEN lakeDeletedTimestamp IS NOT NULL THEN {batch_effective_datetime} ELSE NULL 
          END                               AS lakeDeletedTimestamp
          FROM standardised_mdm.mdm_underwriting_office
          WHERE {batch_effective_datetime} >= lakevalidfromtimestamp
          AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
      ) office_rownum
) office
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwOffice'
  viewTables = 'standardised_mdm.mdm_underwriting_office, standardised_mdm.mdm_fin104_office, standardised_mdm.mdm_claim_office, standardised_mdm.mdm_fronting_office'
