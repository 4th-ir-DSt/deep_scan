...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Signing_Transaction_Narrative_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Signing_Transaction_Narrative table in conformed zone</td></tr>
...
Concat_ws('_', narr.Trnid, narr.Trncgy, narr.Seqlnno)               AS Signing_Message_Narrative_Reference,
COALESCE(nullif(narr.Lnno,''), {missing_integer})                   AS Signing_Message_Line_Number,
COALESCE(nullif(narr.Narr,''), {missing_string})                    AS Signing_Message_Narrative,
Concat_ws('_', usm.Lpsodt, usm.Lpsono, usm.Trnverno)                AS Signing_Reference,
Greatest(narr.Lakelastupdatedate, usm.Lakelastupdatedate)           AS LakeLastUpdateDate,
Greatest(narr.Lakelastupdatetimestamp, usm.Lakelastupdatetimestamp) AS LakeLastUpdateTimestamp,
CASE 
    WHEN coalesce(narr.lakeDeletedTimestamp, usm.lakeDeletedTimestamp) IS NULL
    THEN NULL ELSE {batch_effective_datetime} 
END                                                                 AS lakeDeletedTimestamp
FROM   standardised_subscribe.dbo_usmnarr narr

INNER JOIN standardised_subscribe.dbo_usmmain usm
-- to fetch details from Usm Main table
ON Upper(narr.Trnid) = Upper(usm.Trnid)
AND Upper(narr.Trncgy) = Upper(usm.Trncgy)
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwSigning_Message_Transaction_Narrative'
  viewTables = 'standardised_subscribe.dbo_usmnarr, standardised_subscribe.dbo_usmmain'
