...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_party_policy_header_role_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Party_Policy_Header_Role table in conformed zone</td></tr>
...
SELECT polid,
       ...
       role_type,
       party_code,
       main_party_policy_header_role_indicator,
       MAX(lakelastupdatedate)                                                                 AS lakelastupdatedate,
       MAX(lakelastupdatetimestamp)                                                            AS lakelastupdatetimestamp,
       NULLIF(MAX(COALESCE(lakeDeletedTimestamp,'9999-12-31T01:01:01')),'9999-12-31T01:01:01') AS lakeDeletedTimestamp,
       psu
...
(
    SELECT polinsd.polid,
        ...
        CASE
               ...
               ELSE 'N'
        END                                                                                    AS main_party_policy_header_role_indicator,
    ...
    SELECT main.polid,
           ...
           'N'                                                                                 AS main_party_policy_header_role_indicator,
    ...
    SELECT main.polid,
           ...
           'Y'                                                                                 AS main_party_policy_header_role_indicator,
...
SELECT 
    ...
    CASE 
        WHEN tpu.ROLE_TYPE IN ('MARKET_LEAD', 'LLOYDS_LEAD', 'SLIP_LEAD', 'AGREEMENT_PARTY', 'UNDERWRITER', 'BROKER') 
            THEN tpu.ROLE_TYPE
    ...
    CASE 
        ...
        ELSE COALESCE(TPU.Main_Party_Policy_Header_Role_Indicator,'N') 
    END                                                                                         AS Main_Party_Policy_Header_Role_Indicator, 
    GREATEST(MAIN.LakeLastUpdateDate, TPU.LakeLastUpdateDate, 
    ip.LakeLastUpdateDate, UpdMain1.LakeLastUpdateDate)                                         AS LakeLastUpdateDate,
    GREATEST(MAIN.LakeLastUpdateTimestamp, TPU.LakeLastUpdateTimestamp, 
    ip.LakeLastUpdateTimestamp, UpdMain1.LakeLastUpdateTimestamp)                               AS LakeLastUpdateTimestamp,
    CASE 
         WHEN coalesce(MAIN.lakeDeletedTimestamp, TPU.lakeDeletedTimestamp, ip.lakeDeletedTimestamp) IS NOT NULL THEN {batch_effective_datetime} 
         ELSE NULL 
    END                                                                                         AS lakeDeletedTimestamp
FROM tempPartyUniverse TPU
    ...
    -- to update the Main_Party_Policy_Header_Role_Indicator with Flag "1"
    LEFT JOIN  standardised_subscribe.dbo_PolMain UpdMain1
    ON  UPPER(TPU.PolId) = UPPER(UpdMain1.PolId) AND UPPER(TPU.UnitPsu)= UPPER(UpdMain1.UnitPsu)
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwParty_Policy_Header_Role'
