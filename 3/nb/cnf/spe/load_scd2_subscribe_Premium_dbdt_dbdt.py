...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Premium_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Premium table in conformed zone</td></tr>
...
sqlQuery= """
SELECT 
    Concat(pm.polid, '_', pm.unitpsu)                                              AS Policy_Section_Reference,
    Concat(pm.polid, '_', pm.unitpsu)                                              AS Policy_Header_Reference,
    Concat(pm.polid, '_', pm.unitpsu)                                              AS Coverage_Reference,
        --CASE WHEN nullif(pm.pmccy,'') IN ( 'USD', 'GBP', 'CAD', 'EUR' ) THEN pm.pmccy ELSE 'GBP' END
    COALESCE(CC, 'GBP')                                                            AS Settlement_Currency,
    COALESCE(pm.PmCcy, 'GBP')                                                      AS Original_Currency,
    CASE
        WHEN pm.wtnln = 0 THEN 0.00 ELSE cast(pm.pmamt/NULLIF(pm.wtnln,0) as decimal(28,2))
    END                                                                            AS Market_Gross_Gross_Estimated_Premium,
        --CASE WHEN nullif(pm.pmccy,'') IN ( 'USD', 'GBP', 'CAD', 'EUR' ) THEN 1 ELSE 
        --CASE WHEN roe.lakeDeletedTimestamp is not null then NULL ELSE roe.roe END END
    CAST(roe.roe AS DECIMAL(28,18))                                                AS Multiplication_Rate_of_Exchange,
        --CASE WHEN nullif(pm.pmccy,'') IN ( 'USD', 'GBP', 'CAD', 'EUR' ) THEN 1 ELSE 
        --CASE WHEN roe.lakeDeletedTimestamp is not null THEN NULL ELSE 1/roe.roe END END
    cast(1/NULLIF(roe.roe,0) as decimal(28,18))                                    AS Division_Rate_Of_Exchange,
    GREATEST(pm.lakelastupdatedate, ip.lakelastupdatedate, mds_curr.lakelastupdatedate,
        roe.lakelastupdatedate)                                                    AS LakeLastUpdateDate,
    GREATEST(pm.lakelastupdatetimestamp, ip.lakelastupdatetimestamp, mds_curr.lakelastupdatetimestamp,
        roe.lakelastupdatetimestamp)                                               AS LakeLastUpdateTimestamp,
    CASE 
        WHEN coalesce(pm.lakeDeletedTimestamp, ip.lakeDeletedTimestamp) IS NULL THEN NULL ELSE {batch_effective_datetime} 
...
-- To fetch inwards policy
INNER JOIN standardised_subscribe.dbo_inpol ip
ON Upper(pm.polid) = Upper(ip.polid)
AND Upper(pm.unitpsu) = Upper(ip.unitpsu)
...
-- To identify Currencies that exists in MDS
LEFT JOIN
(
    SELECT PolId, UnitPsu, UPPER(Currency_Code) AS CC,
...
ON UPPER(pm.polid) = UPPER(mds_curr.polid)
AND pm.unitpsu = mds_curr.unitpsu
AND UPPER(pm.PmCcy) = mds_curr.CC

--to fetch rate of exchange for the policy
LEFT JOIN
(
    SELECT CcyPsuFr, CcyPsuTo, CASE WHEN CcyPsuFr = CcyPsuTo THEN 1 ELSE ROE END AS ROE, 
    lakelastupdatedate, lakelastupdatetimestamp
    FROM standardised_subscribe.dbo_roe
    WHERE {batch_effective_datetime} >= lakeValidFromTimestamp 
    AND {batch_effective_datetime} < coalesce(lakeValidToTimestamp, current_timestamp())
    AND lakeDeletedTimestamp IS NULL
) roe
ON  COALESCE(Upper(pm.PmCcy), 'GBP') = Upper(roe.CcyPsuFr)
AND COALESCE(CC, 'GBP') = UPPER(roe.CcyPsuTo)

--ON Upper(pm.pmccy) = Upper(roe.ccypsufr)
--AND CASE
--        WHEN pm.pmccy IN ( 'USD', 'GBP', 'CAD', 'EUR' ) THEN pm.pmccy
--        ELSE 'GBP'
--    END = roe.ccypsuto
--AND {batch_effective_datetime} >= roe.LakeValidFromTimestamp 
--AND {batch_effective_datetime} < coalesce(roe.LakeValidToTimestamp, current_timestamp())
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwPremium'
  viewTables = 'standardised_subscribe.dbo_polmain, standardised_subscribe.dbo_inpol, standardised_subscribe.dbo_roe, standardised_mdm.mdm_Entity_Currency_Type, standardised_subscribe.dbo_inpolptpt'
