...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Policy_Trust_Fund_Relationship_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Policy_Trust_Fund_Relationship table in conformed zone</td></tr>
...
sqlQuery= """
SELECT Coalesce(ptf_ref.Conformed_Code, 
            Concat({missing_mdm},' - ',COALESCE(NULLIF(Trim(pac.cd),''), {missing_string})))            AS Trust_Fund_Reference,
       Concat(pac.polid, '_', pac.unitpsu)                                                              AS Policy_Section_Reference,
       Concat(pac.polid, '_', pac.unitpsu)                                                              AS Policy_Header_Reference,
       Greatest(pac.lakelastupdatedate, main.lakelastupdatedate, ip.lakelastupdatedate, 
               ptf_ref.lakelastupdatedate)                                                              AS LakeLastUpdateDate,
       Greatest(pac.lakelastupdatetimestamp, main.lakelastupdatetimestamp, ip.lakelastupdatetimestamp, 
               ptf_ref.lakelastupdatetimestamp)                                                         AS LakeLastUpdateTimestamp,
       CASE 
           WHEN coalesce(pac.lakeDeletedTimestamp, main.lakeDeletedTimestamp, ip.lakeDeletedTimestamp) IS NULL
           THEN NULL ELSE {batch_effective_datetime} 
       END                                                                                              AS lakeDeletedTimestamp
FROM standardised_subscribe.dbo_polanlycd pac

-- to fetch policies from Analyst Code that exists in Main table
INNER JOIN standardised_subscribe_dbo_polmain_tmpvw main
ON Upper(pac.polid) = Upper(main.polid)
AND Upper(pac.unitpsu) = Upper(main.unitpsu)
--AND Upper(pac.ty) = 'TFC'
...
ON Upper(pac.polid) = Upper(ip.polid)
AND Upper(pac.unitpsu) = Upper(ip.unitpsu)
...
-- MDS feild: Trust_Fund_Reference
LEFT JOIN standardised_mds.dbo_policy_trust_fund_reference ptf_ref
ON main.Polty = ptf_ref.Code
AND lower(ptf_ref.Name) = 'subscribe' 
AND {batch_effective_datetime} >= Main.LakeValidFromTimestamp 
AND {batch_effective_datetime} < coalesce(Main.LakeValidToTimestamp, current_timestamp())
AND {batch_effective_datetime} >= ptf_ref.LakeValidFromTimestamp 
AND {batch_effective_datetime} < coalesce(ptf_ref.LakeValidToTimestamp, ptf_ref.lakeDeletedTimestamp, current_timestamp())

WHERE Upper(pac.ty) = ({string_filter})
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwPolicy_Trust_Fund_Relationship'
  viewTables = 'standardised_Subscribe.dbo_PolMain,standardised_subscribe.dbo_polanlycd, standardised_subscribe.dbo_inpol,standardised_mds.dbo_policy_trust_fund_reference'
...
try:
    std_object = 'standardised_subscribe.dbo_polmain'
    ...
    union_df.createOrReplaceTempView('standardised_subscribe_dbo_polmain_tmpvw')
    
    logTaskProgress(cursor,batchTaskId,"standardised_subscribe_dbo_polmain_tmpvw created")
