...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_clause_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Clause table in conformed zone</td></tr>
# MAGIC   <tr><td><b>Author</b></td><td>Akhilesh</td></tr>
# MAGIC   <tr><td><b>Creation Date</b></td><td>2022/02/16</td></tr>
# MAGIC </table>
# MAGIC 
# MAGIC <table>
# MAGIC   <tr>
...
# MAGIC   </tr>
# MAGIC   <tr>
# MAGIC     <td>2022/04/07</td>
# MAGIC     <td>Akhilesh</td>
# MAGIC     <td>lakevalidfromtimestamp,lakevalidtotimestamp & lakeisactive columns removed from query, these will be handled by applySCDType2onDestForBatchLoad & addAuditAndHBKColumnsOnDF functions.<br>
# MAGIC       UNION of 2 queries merged to a single query by removing condition LakeIsActive as batch effective date condition is sufficient
# MAGIC     </td>
# MAGIC   </tr>  
# MAGIC   <tr>
# MAGIC     <td>2022/05/05</td>
# MAGIC     <td>Akhilesh</td>
# MAGIC     <td>lakeDeletedTimestamp column related changes added
# MAGIC     </td>
# MAGIC   </tr>  
# MAGIC </table>
# MAGIC 
...
sqlQuery = """
SELECT     
    Concat(a.polid,'_',a.unitpsu)                                                                 AS policy_section_reference,
    Concat(a.polid,'_',a.unitpsu)                                                                 AS policy_header_reference,  
    COALESCE(NULLIF(Trim(a.sectty),''),{missing_string})                                          AS Source_Clause_Type,
    COALESCE(NULLIF(Trim(a.sectdsc),''),{missing_string})                                         AS clause_description,
    CASE 
        WHEN ct_ref.lakeDeletedTimestamp is not null THEN {missing_mdm} 
        ELSE COALESCE(NULLIF(Trim(ct_ref.Clause_Type_Code),''),{missing_mdm}) 
    END                                                                                           AS Clause_Type,
    greatest(a.lakelastupdatedate, main.lakelastupdatedate,ip.lakelastupdatedate,
        ct_ref.lakelastupdatedate)                                                                AS lakelastupdatedate,
    greatest(a.lakelastupdatetimestamp, main.lakelastupdatetimestamp,ip.lakelastupdatetimestamp,
        ct_ref.lakelastupdatetimestamp)                                                           AS lakelastupdatetimestamp,
    CASE 
        WHEN coalesce(a.lakeDeletedTimestamp, main.lakeDeletedTimestamp, ip.lakeDeletedTimestamp) IS NULL THEN NULL
...
FROM (
        ...
        WHERE    upper(sectty) IN ({string_filter_PolSectNarr})
...
-- to fetch only those policies FROM SectNarr that exist is in Main table
INNER JOIN standardised_subscribe.dbo_polmain main
ON         upper(a.polid) = upper(main.polid)
AND        upper(a.unitpsu) = upper(main.unitpsu)
...
ON          upper(a.polid) = upper(ip.polid)
AND         upper(a.unitpsu) = upper(ip.unitpsu)
...
AND        {batch_effective_datetime} < COALESCE(ip.lakevalidtotimestamp, CURRENT_TIMESTAMP())
           ...
           ---MDS Fields: Clause Type
LEFT JOIN  standardised_mdm.mdm_clause_type_mapping ct_ref
ON         a.sectty = ct_ref.Source_Clause_Type
AND        lower(ct_ref.Source_Name) = 'subscribe'
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwClause'
  viewTables = 'standardised_Subscribe.dbo_PolMain,standardised_Subscribe.dbo_PolSectNarr, standardised_subscribe.dbo_inpol,standardised_mdm.mdm_clause_type_mapping'
...
try:
    ...
    chk_column = 'sectty'
    apply_upper_trim = "y"
    filter_list = ['CAS_PREMIUM PAYMENT WARRANTY',
                 'SRCC EXCLUSION',
                 'COMMUNICABLE DISEASE EXCLUSION',
                 'CYBER EXCLUDED',
                 'CYBER EXCLUSION REFERENCE',
                 'PREMIUM PAYMENT WARRANTY (PPW)',
                 'TERRORISM AND WAR PERILS CODES',
                 'CAS_WAR AND TERROR',
                 'CYBER']
...
try:
      if dependentTableIsUpdated == True:
            string_filter_PolSectNarr = "','".join(filter_list)
            ...
            sqlQuery = sqlQuery.format(batch_effective_datetime = "'" + str(batchEffectiveDatetime) + "'", 
                                       ...
                                       string_filter_PolSectNarr = "'" +str(string_filter_PolSectNarr) + "'",
