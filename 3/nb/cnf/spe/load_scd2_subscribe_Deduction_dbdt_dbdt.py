...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_deduction_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Deduction table in conformed zone</td></tr>
...
sqlQuery ="""
SELECT
CONCAT(deds.polid, '_', deds.unitpsu)                                                        AS Policy_Section_Reference,
CONCAT(deds.polid, '_', deds.unitpsu)                                                        AS Policy_Header_Reference,
COALESCE(NULLIF(TRIM(deds.dedty),''),{missing_string})                                       AS Source_Deduction_Type_Code,
CONCAT(deds.polid, '_', deds.unitpsu)                                                        AS Coverage_Reference,
CASE 
    WHEN ded_typ_map.lakeDeletedTimestamp IS NOT NULL THEN {missing_mdm} 
    ELSE COALESCE(ded_typ_map.deduction_type_code,{missing_mdm}) 
END                                                                                          AS Deduction_Type_code, 
CASE
    WHEN ded_typ_ref.lakeDeletedTimestamp IS NOT NULL THEN {missing_mdm} 
    ELSE coalesce(ded_typ_ref.Deduction_Type_Name, {missing_mdm})
END                                                                                          AS Deduction_Type_Name , 
CASE
    WHEN ded_typ_ref.lakeDeletedTimestamp IS NOT NULL THEN {missing_mdm} 
    ELSE coalesce(ded_typ_ref.Deduction_Type_Group_Name, {missing_mdm})
END                                                                                          AS Deduction_Type_Group,
COALESCE(NULLIF(TRIM(deds.grnet), ''), 'G')                                                  AS Applied_To_Gross_Or_Net_Indicator,
'1'                                                                                          AS Deduction_Sequence,
CASE
    WHEN COALESCE(deds.dedamt, 0.00) <= 0 THEN 0
    WHEN deds.dedamt > 100 THEN 100
    ELSE CAST(deds.dedamt AS DECIMAL(19,8))
END                                                                                          AS Deduction_Percentage,
{missing_string}                                                                             AS Deduction_Currency,
CAST(0.00 AS DECIMAL(28,2))                                                                  AS Line_Deduction,      
GREATEST( deds.lakelastupdatedate, main.lakelastupdatedate, ip.lakelastupdatedate,	
    ded_typ_map.lakelastupdatedate, ded_typ_ref.lakelastupdatedate)                          AS lakelastupdatedate,
GREATEST( deds.lakelastupdatetimestamp, main.lakelastupdatetimestamp, ip.lakelastupdatetimestamp, 
	ded_typ_map.lakelastupdatetimestamp, ded_typ_ref.lakelastupdatetimestamp)                AS lakelastupdatetimestamp,
CASE
    WHEN COALESCE(deds.lakeDeletedTimestamp, main.lakeDeletedTimestamp, ip.lakeDeletedTimestamp) IS NULL
    THEN NULL ELSE {batch_effective_datetime} 
END                                                                                          AS lakeDeletedTimestamp   
FROM standardised_subscribe.dbo_polpmdeds deds

-- to fetch only those policies from PremiumDeduction that exists in Main table
INNER JOIN standardised_subscribe.dbo_polmain main
ON UPPER(deds.polid) = UPPER(main.polid)
AND UPPER(deds.unitpsu) = UPPER(main.unitpsu)
AND {batch_effective_datetime} >= deds.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(deds.lakevalidtotimestamp, CURRENT_TIMESTAMP())
...
ON  UPPER(deds.polid) =  UPPER(ip.polid)
AND UPPER(deds.unitpsu) =  UPPER(ip.unitpsu)
...
-- Join polpmdeds with MDM Mapping table to obtain MDM Deduction_Type_Code 
LEFT JOIN standardised_mdm.mdm_deduction_type_mapping ded_typ_map 
ON deds.DedTy = ded_typ_map.Source_Deduction_Type_Code 
AND LOWER(ded_typ_map.Source_Name) = 'subscribe'
AND {batch_effective_datetime} >= ded_typ_map.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(ded_typ_map.lakevalidtotimestamp, CURRENT_TIMESTAMP())    

-- Join MDM Mapping table to MDM_Deduction_Type to obtain Deduction type an Deduction Group names 
LEFT JOIN standardised_mdm.mdm_deduction_type ded_typ_ref 
ON ded_typ_ref.deduction_type_code = ded_typ_map.deduction_type_code
AND {batch_effective_datetime} >= ded_typ_ref.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(ded_typ_ref.lakevalidtotimestamp, CURRENT_TIMESTAMP())
     
WHERE {batch_effective_datetime} >= deds.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(deds.lakevalidtotimestamp, CURRENT_TIMESTAMP())
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwDeduction'
  viewTables = 'standardised_Subscribe.dbo_polpmdeds, standardised_Subscribe.dbo_polmain, standardised_subscribe.dbo_inpol, standardised_mdm.mdm_deduction_type_mapping, standardised_mdm.mdm_deduction_type'
