...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Claim_Line_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Claim_Line table in conformed zone</td></tr>
# MAGIC   <tr><td><b>Author</b></td><td>Rahul</td></tr>
# MAGIC   <tr><td><b>Creation Date</b></td><td>2022/04/11</td></tr>
# MAGIC </table>
# MAGIC 
# MAGIC 
# MAGIC <table>
...
sqlQuery= """
SELECT 
concat_ws('_',upper(trim(main.TrnId)),upper(trim(main.TrnCgy)),trim(movt.MovtSeqNo))                  AS Claim_Line_Reference,
upper(Concat(COALESCE(TrnAnly.AnlyCd,main.polid), '_', main.unitpsu))                                 AS Policy_Header_Reference,
upper(Concat(COALESCE(TrnAnly.AnlyCd,main.polid), '_', main.unitpsu))                                 AS Policy_Section_Reference,
upper(Concat(COALESCE(TrnAnly.AnlyCd,main.polid), '_', main.unitpsu))                                 AS Coverage_Reference,
upper(Concat(COALESCE(TrnAnly.AnlyCd,main.polid), '_', main.unitpsu))                                 AS Item_Reference,
COALESCE(upper(nullif(trim(main.BPR), '')),{missing_string})                                          AS Claim_Component_Reference,
concat_ws('_',upper(trim(main.TrnId)),upper(trim(main.TrnCgy)),trim(movt.MovtSeqNo))                  AS Claim_Transaction_Reference,
COALESCE(main.UnitId, {missing_string})                                                               AS Business_Entity_Code,
CASE 
     WHEN busi_clas_ref.lakeDeletedTimestamp is not null then {missing_mdm} 
     ELSE COALESCE(busi_clas_ref.Claim_Office_Code, {missing_mdm}) 
END                                                                                                   AS Office_Code,
CASE 
     WHEN Sgnd.lakeDeletedTimestamp IS NULL AND Sgnd.SgndLn > 0.00 THEN Sgnd.SgndLn 
     WHEN ip.lakeDeletedTimestamp IS NULL AND (ip.SgndOrdPctg * inpolpt.SpltSgndLn)/100 > 0.00 
         THEN CAST((ip.SgndOrdPctg * inpolpt.SpltSgndLn)/100 AS DECIMAL(19,8))
     WHEN polmain.lakeDeletedTimestamp IS NULL AND (polmain.WtnOrdPctg * inpolpt.SpltWtnLn)/100 > 0.00 
         THEN CAST((polmain.WtnOrdPctg * inpolpt.SpltWtnLn)/100 AS DECIMAL(19,8))
     ELSE 0
END                                                                                                   AS Claim_Line,
greatest(main.LakeLastUpdateDate, Sgnd.LakeLastUpdateDate, movt.LakeLastUpdateDate, inpolpt.LakeLastUpdateDate,
    cvr.LakeLastUpdateDate, ip.LakeLastUpdateDate, busi_clas_ref.LakeLastUpdateDate, TrnAnly.LakeLastUpdateDate,
    polmain.LakeLastUpdateDate)                                                                       AS LakeLastUpdateDate,
greatest(main.LakeLastUpdateTimestamp, Sgnd.LakeLastUpdateTimestamp, movt.LakeLastUpdateTimestamp, inpolpt.LakeLastUpdateTimestamp,
    cvr.LakeLastUpdateTimestamp, ip.LakeLastUpdateTimestamp, busi_clas_ref.LakeLastUpdateTimestamp, TrnAnly.LakeLastUpdateTimestamp,
    polmain.LakeLastUpdateTimestamp)                                                                  AS LakeLastUpdateTimestamp,
CASE 
    WHEN coalesce(main.lakeDeletedTimestamp, movt.lakeDeletedTimestamp) IS NOT NULL THEN {batch_effective_datetime} ELSE NULL 
...
--to fetch Claim_Transaction_Reference
INNER JOIN standardised_subscribe.dbo_SCMMovt movt
ON  upper(main.TrnId) = upper(movt.TrnId)
AND upper(main.TrnCgy) = upper(movt.TrnCgy)
AND {batch_effective_datetime} >= movt.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(movt.lakevalidtotimestamp, CURRENT_TIMESTAMP())

--to fetch Policy Related Details
LEFT JOIN
(
    SELECT TrnId, TrnCgy, MovtSeqNo, AnlyCd, lakelastupdatedate, lakelastupdatetimestamp
    from standardised_subscribe.dbo_TrnAnly
    WHERE UPPER(TRIM(AnlyTy)) = 'POLID' AND TrnCgy = 'SCM' AND lakeDeletedTimestamp IS NULL
    AND {batch_effective_datetime} >= lakevalidfromtimestamp
    AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
) TrnAnly
ON  upper(movt.TrnId)  = upper(TrnAnly.TrnId)
AND upper(movt.TrnCgy) = upper(TrnAnly.TrnCgy)
AND upper(movt.MovtSeqNo) = upper(TrnAnly.MovtSeqNo)

--- To fetch the SgndLn for maximum TrnSeqNo
LEFT JOIN
(
     SELECT
     SgndLn,
     ...
     FROM
     (
         SELECT row_number() OVER(partition BY BPR, SLN, CKY, UnitID 
         ORDER BY coalesce(lakeDeletedTimestamp, '9999-12-31T01:01:01.000') desc, TrnSeqNo desc) AS rnk, *
...
ON  main.BPR = Sgnd.BPR
AND main.SLN = Sgnd.SLN
AND main.CKY = Sgnd.CKY 
AND main.UnitID = Sgnd.UnitID

--to fetch coverage type of policies that exists in universe/main table
LEFT JOIN  
(
    select polid, unitpsu, ty, cd, LakeLastUpdateTimestamp, LakeLastUpdateDate ,lakeDeletedTimestamp
    from standardised_subscribe.dbo_polanlycd 
    where upper(trim(ty)) = 'SUBCLASS'
    AND {batch_effective_datetime} >= lakevalidfromtimestamp
    AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
) cvr
ON   upper (COALESCE(TrnAnly.AnlyCd,main.polid)) = upper(cvr.polid)
AND  upper (main.unitpsu) = upper(cvr.unitpsu)

-- To fetch the AcctgYr from inpol to join with Year in mdm
LEFT JOIN
(
     select polid, unitpsu,SgndOrdPctg, AcctgYr, LakeLastUpdateTimestamp, LakeLastUpdateDate ,lakeDeletedTimestamp
     from standardised_subscribe.dbo_inpol
     WHERE {batch_effective_datetime} >= lakevalidfromtimestamp
     AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
) ip
ON  upper (COALESCE(TrnAnly.AnlyCd,main.polid)) = upper (ip.polid)
AND upper (main.unitpsu) = upper (ip.unitpsu)
 
LEFT JOIN
(
     select PolId,UnitPsu,WtnOrdPctg , LakeLastUpdateTimestamp, LakeLastUpdateDate ,lakeDeletedTimestamp
     from standardised_subscribe.dbo_polmain
     WHERE {batch_effective_datetime} >= lakevalidfromtimestamp
     AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
) polmain
ON  upper (COALESCE(TrnAnly.AnlyCd,main.polid)) = upper (polmain.PolId)
AND upper (main.unitpsu) = upper (polmain.UnitPsu)
 
LEFT JOIN
(
     select PolId, UnitPsu, unitid, SpltSgndLn, SpltWtnLn, LakeLastUpdateTimestamp, LakeLastUpdateDate ,lakeDeletedTimestamp
     from standardised_subscribe.dbo_inpolptpt
     WHERE {batch_effective_datetime} >= lakevalidfromtimestamp
     AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
) inpolpt
ON  upper (COALESCE(TrnAnly.AnlyCd,main.polid)) = upper (inpolpt.PolId)
AND upper (main.unitpsu) = upper (inpolpt.UnitPsu)
AND upper (main.unitid) = upper (inpolpt.unitid)
 
-- MDM Feild: Office_Code
LEFT JOIN standardised_mdm.mdm_business_classification_mapping busi_clas_ref
ON upper(cvr.cd) = upper(busi_clas_ref.Sub_Class_Code)
AND (ip.AcctgYr) = (busi_clas_ref.Year)
AND {batch_effective_datetime} >= busi_clas_ref.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(busi_clas_ref.lakevalidtotimestamp, CURRENT_TIMESTAMP())
"""
...
try:
  ...
  #standardised_subscribe.dbo_PolAnlyCd, standardised_mdm.mdm_business_entity, 
  viewName = 'dummy_Dummy.dummyVwClaim_Line'
  viewTables = 'standardised_subscribe.dbo_scmmain, standardised_subscribe.dbo_SCMMovt, standardised_mdm.mdm_business_classification_mapping, standardised_subscribe.dbo_inpol, standardised_subscribe.dbo_polmain, standardised_subscribe.dbo_inpolptpt, standardised_subscribe.dbo_TrnAnly, standardised_subscribe.dbo_polanlycd'
