...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Party_Claim_Component_Role_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Party_Claim_Component_Role table in conformed zone</td></tr>
...
sqlQuery="""
SELECT 
upper(nullif(trim(main.BPR), ''))                                                     AS Claim_Component_Reference,
Concat(Trim(Upper(narr.sectdsc)), '_', {missing_string}, '_',upd_SectTy)              AS Party_Reference,
upd_SectTy                                                                            AS Party_Role_Reference,
COALESCE(NULLIF(upd_SectTy, ''), {missing_string})                                    AS Role_Type,
'Y'                                                                                   AS Main_Party_Claim_Component_Role_Indicator,
max(Greatest(movt.lakelastupdatedate, main.lakelastupdatedate, 
    narr.lakelastupdatedate))                                                         AS lakelastupdatedate,
max(Greatest(movt.lakelastupdatetimestamp, main.lakelastupdatetimestamp, 
    narr.lakelastupdatetimestamp))                                                    AS lakelastupdatetimestamp,
CASE 
   WHEN max(coalesce(movt.lakeDeletedTimestamp, main.lakeDeletedTimestamp, narr.lakeDeletedTimestamp)) IS NOT NULL 
...
--bringing in UCR from the main table
INNER JOIN standardised_subscribe_dbo_scmmain_tmpvw main
...
--bringing details from narrative table
INNER JOIN 
(
    SELECT TrnId, TrnCgy, 
    CASE 
        WHEN upper(trim(SectTy)) = 'INSURED' THEN 'CLAIM_INSURED'
        WHEN upper(trim(SectTy)) = 'LOSS ADJUSTER' THEN 'CLAIM_HANDLER'
        WHEN upper(trim(SectTy)) = 'BROKER' THEN 'CLAIM_BROKER'
        WHEN upper(trim(SectTy)) = 'ORIGINAL INSURED' THEN 'CLAIM_ORIGINAL_INSURED'
        WHEN upper(trim(SectTy)) = 'CLAIMANT' THEN 'CLAIM_CLAIMANT'
        WHEN upper(trim(SectTy)) = 'BUREAU LEAD' THEN 'CLAIM_BUREAU_LEAD'
        WHEN upper(trim(SectTy)) = 'LAWYER' THEN 'CLAIM_LAWYER'
        ELSE upper(trim(SectTy))
    END AS upd_SectTy, SectDsc, lakelastupdatedate, Lakelastupdatetimestamp, lakeDeletedTimestamp,
    row_number() OVER(partition BY TrnId, TrnCgy, SectTy 
        ORDER BY coalesce(lakeDeletedTimestamp, '9999-12-31T01:01:01.000') desc, TrnSectNarrId DESC) AS rnk
    FROM standardised_subscribe_dbo_trnsectnarr_tmpvw
    WHERE trim(upper(sectty)) IN ({string_filter_trnsectnarr})
    AND {batch_effective_datetime} >= LakeValidFromTimestamp  
    AND {batch_effective_datetime} < COALESCE(LakeValidToTimestamp, CURRENT_TIMESTAMP())
 ) narr
ON  LEFT(main.trnid, Length(main.trnid) - 3) = narr.trnid
AND main.Trncgy = narr.Trncgy
AND rnk = 1

-- We are showing data at higher granularity, thus grouping the records 
GROUP  BY 1, 2, 3, 4
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwParty_Claim_Component_Role'
  viewTables = 'standardised_subscribe.dbo_scmmain, standardised_subscribe.dbo_trnsectnarr, standardised_subscribe.dbo_scmmovt'
...
# DBTITLE 1,Create Filter Standardised View For dbo_trnsectnarr
try:
    std_object_trnsectnarr = 'standardised_subscribe.dbo_trnsectnarr'
    current_bed = batchEffectiveDatetime
    chk_column_trnsectnarr = 'sectty'
    apply_upper_trim_trnsectnarr = "y"
    filter_list_trnsectnarr = ['INSURED',
                               'LAWYER',
                               'BROKER',
                               'LOSS ADJUSTER',
                               'ORIGINAL INSURED',
    ...
    if prv_bed is not None and dependentTableIsUpdated == True:
        std_df_trnsectnarr = createStandardisedFilterView(std_object_trnsectnarr,
                                                          ...
                                                          current_bed,
                                                          whereExpression,
                                                          chk_column_trnsectnarr,
                                                          apply_upper_trim_trnsectnarr,
                                                          filter_list_trnsectnarr,
                                                          cursor,
                                                          batchTaskId,
                                                          adfPipelineName,
                                                          clusterId,
...
# DBTITLE 1,Create standardised Grain View For dbo_scmmain
try:
    std_object_scmmain = 'standardised_subscribe.dbo_scmmain'
    current_bed = batchEffectiveDatetime
    chk_column_list_scmmain = ['BPR']
...
# DBTITLE 1,Create standardised Grain View For dbo_scmmovt
try:
    std_object_scmmovt = 'standardised_subscribe.dbo_scmmovt'
    current_bed = batchEffectiveDatetime
    chk_column_list_scmmovt = ['BPR']
    
    if prv_bed is not None and dependentTableIsUpdated == True:
        std_df_scmmovt=createStandardisedGrainView(std_object_scmmovt,
                                                   prv_bed,
                                                   current_bed,
                                                   whereExpression,
                                                   chk_column_list_scmmovt,
                                                   cursor,
                                                   batchTaskId,
                                                   adfPipelineName,
                                                   clusterId,
...
try:
    if dependentTableIsUpdated == True:
        ...
        string_filter_trnsectnarr="','".join(filter_list_trnsectnarr)
        quote_String_trnsectnarr="'"+string_filter_trnsectnarr+"'"
        
        sqlQuery = sqlQuery.format(batch_effective_datetime = "'" + str(batchEffectiveDatetime) + "'", 
                                   missing_string = "'" +str(missing_string) + "'",
                                   string_filter_trnsectnarr=str(quote_String_trnsectnarr))
