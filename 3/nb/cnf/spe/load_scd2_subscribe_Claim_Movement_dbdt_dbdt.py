...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Claim_Movement_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Claim_Movement table in conformed zone</td></tr>
# MAGIC   <tr><td><b>Author</b></td><td>Kaveti</td></tr>
# MAGIC   <tr><td><b>Creation Date</b></td><td>2022/04/08</td></tr>
# MAGIC </table>
# MAGIC 
# MAGIC 
# MAGIC <table>
...
sqlQuery="""
SELECT 
    concat_ws('_',trim(upper(movt.TrnId)),trim(upper(movt.TrnCgy)), movt.movtseqno)                       AS Claim_Movement_Reference,
    upper(concat_ws('_',trim(movt.TrnId), trim(movt.TrnCgy)))                                             AS Claim_Transaction_Reference,
    movt.Movtseqno                                                                                        AS Claim_Movement_Number,
    movt.Dttm                                                                                             AS Claim_Advice_Date,
    movt.Accyr                                                                                            AS Year_Of_Account,
    cast(movt.Accprd as int)                                                                              AS Source_Accounting_Period,
    CASE                  
           WHEN acc_ref.lakeDeletedTimestamp is not null THEN 190001                  
           ELSE COALESCE(acc_ref.Accounting_Period, 190001)                  
    END                                                                                                   AS Accounting_Period,
    CASE WHEN LEFT(lower(movt.MRN), 1) = 'm' THEN 'Y' ELSE 'N' END                                        AS Manual_Claim_Advice_Indicator,
    {missing_string}                                                                                      AS Claim_Movement_Status,
    Coalesce(Nullif(Trim(main.CMCD), ''), {missing_string})                                               AS Claim_Movement_Type,
    Coalesce(REPLACE(main.CurrNarrA, 'dc937b59892', {missing_string}), {missing_string})                  AS Claim_Advice_Narrative_1,
    Coalesce(REPLACE(main.CurrNarrB, 'dc937b59892', {missing_string}), {missing_string})                  AS Claim_Advice_Narrative_2,
    Greatest(movt.Lakelastupdatedate, main.Lakelastupdatedate, acc_ref.Lakelastupdatedate)                AS lakelastupdatedate,
    Greatest(movt.Lakelastupdatetimestamp, main.Lakelastupdatetimestamp, acc_ref.Lakelastupdatetimestamp) AS lakelastupdatetimestamp,
    CASE 
        WHEN coalesce(movt.lakeDeletedTimestamp, main.lakeDeletedTimestamp) IS NOT NULL 
        THEN {batch_effective_datetime} ELSE NULL 
    END                                                                                                   AS lakeDeletedTimestamp
FROM   
(
    SELECT * from standardised_subscribe.dbo_scmmovt 
...
INNER JOIN standardised_subscribe.dbo_scmmain main
...
-- MDM entity: Accounting period
LEFT JOIN standardised_mdm.mdm_accounting_period_mapping acc_ref
ON lower(acc_ref.Source_Code) = 'subscribe'
AND cast(movt.Accprd as int) = acc_ref.Accounting_Period
AND {batch_effective_datetime} >= acc_ref.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(acc_ref.lakevalidtotimestamp, CURRENT_TIMESTAMP())
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwClaim_Movement'
  viewTables = 'standardised_subscribe.dbo_scmmovt, standardised_subscribe.dbo_scmmain, standardised_mdm.mdm_accounting_period_mapping'
...
try:
  if dependentTableIsUpdated == True:
    ...
    sqlQuery = sqlQuery.format(batch_effective_datetime = "'" + str(batchEffectiveDatetime) + "'",
                               missing_integer = "'" + str(missing_integer) + "'",
