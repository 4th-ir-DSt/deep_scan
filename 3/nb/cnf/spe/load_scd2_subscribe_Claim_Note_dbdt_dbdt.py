...
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Claim_Note table in conformed zone</td></tr>
# MAGIC   <tr><td><b>Author</b></td><td>Kaveti</td></tr>
# MAGIC   <tr><td><b>Creation Date</b></td><td>2022/04/21</td></tr>
# MAGIC </table>
# MAGIC 
# MAGIC 
# MAGIC <table>
...
try:
  import pandas as pd
  import pyodbc
  from pyspark.sql.functions import rank,desc,col,lag, when, lit, concat,row_number,collect_list, coalesce, concat_ws, regexp_replace
...
select 
dt.Claim_Note_Reference,
dt.Claim_Note_Number,
dt.Claim_Component_Reference,
dt.Claim_Note_Type,
dt.Claim_Note_Description,
dt.Claim_Note_Author,
dt.Claim_Note_Code,
Greatest(dt.lakelastupdatedate, main.lakelastupdatedate)                                     AS lakelastupdatedate,
Greatest(dt.lakelastupdatetimestamp, main.lakelastupdatetimestamp)                           AS lakelastupdatetimestamp,
CASE 
    WHEN coalesce(dt.lakeDeletedTimestamp, main.lakeDeletedTimestamp) IS NOT NULL THEN {batch_effective_datetime} ELSE NULL 
END                                                                                          AS lakeDeletedTimestamp
FROM   
(
    select
    claim_note_reference,
    claim_note_number,
    claim_component_reference,
    claim_note_type,
    claim_note_code,
    coalesce(NULLIF(claim_note_description, ''), {missing_string})  as claim_note_description,
    claim_note_author,
    lakelastupdatedate,
    lakelastupdatetimestamp,
    lakeDeletedTimestamp,
    row_number() OVER(partition BY claim_note_reference 
    ORDER BY coalesce(lakeDeletedTimestamp, '9999-12-31T01:01:01.000') desc, tmstp DESC)     AS rnk
    FROM   
    (
      SELECT 
      concat(Upper(Trim(bpr)), '_', Upper(Trim(mrn)), '_', COALESCE(NULLIF(lossdtlty, ''), 'UNCODED_Loss'), '_', paracd) AS Claim_Note_Reference,
      {missing_string}                                                                       AS Claim_Note_Number,
      upper(nullif(trim(BPR), ''))                                                           AS Claim_Component_Reference,
      coalesce(NULLIF(lossdtlty, ''), {missing_string})                                      AS Claim_Note_Type,
      paracd                                                                                 AS Claim_Note_Code,
      Replace(Concat_ws('_', lossdtl1, lossdtl2, lossdtl3, lossdtl4, lossdtl5), '`', '"')    AS Claim_Note_Description,
      {missing_string}                                                                       AS Claim_Note_Author,
      lakelastupdatedate,
      lakelastupdatetimestamp,
      lakeDeletedTimestamp,
      tmstp
      FROM standardised_subscribe_dbo_scmextdlossdtl_tmpvw
      WHERE ({string_filter_scmextdlossdtl}) 
    ...
    UNION ALL
      ...
      SELECT 
      concat(Upper(Trim(bpr)), '_', Upper(Trim(mrn)), '_', COALESCE(NULLIF(poldtlty, ''), 'UNCODED_Pol'), '_', paracd)   AS Claim_Note_Reference,
      {missing_string}                                                                       AS Claim_Note_Number,
      upper(nullif(trim(BPR), ''))                                                           AS Claim_Component_Reference,
      coalesce(NULLIF(poldtlty, ''), {missing_string})                                       AS Claim_Note_Type,
      paracd                                                                                 AS Claim_Note_Code,
      Replace(Concat_ws('_', poldtl1, poldtl2, poldtl3, poldtl4, poldtl5),'`', '"')          AS Claim_Note_Description,
      {missing_string}                                                                       AS Claim_Note_Author,
      lakelastupdatedate,
      lakelastupdatetimestamp,
      lakeDeletedTimestamp,
      tmstp
      FROM 
      standardised_subscribe_dbo_scmextdpoldtl_tmpvw
      WHERE ({string_filter_scmextdpoldtl})
      AND {batch_effective_datetime} >= lakevalidfromtimestamp
      AND {batch_effective_datetime} < COALESCE(lakevalidtotimestamp,CURRENT_TIMESTAMP())
    ) note
) dt

-- Added the join on 17 Aug 2022 which has all the BPR present in main table
INNER JOIN 
(
    Select BPR, lakelastupdatedate, lakelastupdatetimestamp, lakeDeletedTimestamp,
    row_number() OVER(partition BY BPR 
        ORDER BY coalesce(lakeDeletedTimestamp, '9999-12-31T01:01:01.000') desc)             AS bpr_rnk
    from standardised_subscribe_dbo_scmmain_tmpvw 
    WHERE {batch_effective_datetime} >= lakevalidfromtimestamp
    AND   {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
) main
ON upper(trim(dt.claim_component_reference)) =  upper(nullif(trim(main.BPR), ''))

WHERE dt.rnk = 1 and main.bpr_rnk = 1
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwClaim_Note'
  viewTables = 'standardised_subscribe.dbo_SCMExtdLossDtl, standardised_subscribe.dbo_scmextdpoldtl, standardised_subscribe.dbo_scmmain'
...
try:
    std_object_scmextdlossdtl = 'standardised_subscribe.dbo_scmextdlossdtl'
    current_bed = batchEffectiveDatetime
    string_filter_scmextdlossdtl = """
    Replace(Concat_ws('_', lossdtl1, lossdtl2, lossdtl3, lossdtl4, lossdtl5), '`', '"') IS NOT NULL
    """
    
    if prv_bed is not None and dependentTableIsUpdated == True:
        std_df_scmextdlossdtl = createStandardisedMultipleFilterView(std_object_scmextdlossdtl, 
                                                                     prv_bed,
                                                                     current_bed,
                                                                     whereExpression,
                                                                     string_filter_scmextdlossdtl,
                                                                     ...
                                                                     notebookName,
                                                                     errorLogFileLocation)
        union_df_scmextdlossdtl=spark.read.table(std_object_scmextdlossdtl).unionByName(std_df_scmextdlossdtl)
    else :
        union_df_scmextdlossdtl=spark.read.table(std_object_scmextdlossdtl)
        
    union_df_scmextdlossdtl.createOrReplaceTempView('standardised_subscribe_dbo_scmextdlossdtl_tmpvw')
    
    logTaskProgress(cursor,batchTaskId,"standardised_subscribe_dbo_scmextdlossdtl_tmpvw created")
...
# DBTITLE 1,Create Filter Standardised View
try:
    std_object_scmextdpoldtl = 'standardised_subscribe.dbo_scmextdpoldtl'
    current_bed = batchEffectiveDatetime
    string_filter_scmextdpoldtl = """
    Replace(Concat_ws('_', poldtl1, poldtl2, poldtl3, poldtl4, poldtl5), '`', '"') IS NOT NULL
    """
    
    if prv_bed is not None and dependentTableIsUpdated == True:
        std_df_scmextdpoldtl = createStandardisedMultipleFilterView(std_object_scmextdpoldtl, 
                                                                    prv_bed,
                                                                    current_bed,
                                                                    whereExpression,
                                                                    string_filter_scmextdpoldtl,
                                                                    ...
                                                                    notebookName,
                                                                    errorLogFileLocation)
        union_df_scmextdpoldtl=spark.read.table(std_object_scmextdpoldtl).unionByName(std_df_scmextdpoldtl)
    else :
        union_df_scmextdpoldtl=spark.read.table(std_object_scmextdpoldtl)
        
    union_df_scmextdpoldtl.createOrReplaceTempView('standardised_subscribe_dbo_scmextdpoldtl_tmpvw')
    
    logTaskProgress(cursor,batchTaskId,"standardised_subscribe_dbo_scmextdpoldtl_tmpvw created")
...
try:
    if dependentTableIsUpdated == True:
        ...
        sqlQuery = sqlQuery.format(batch_effective_datetime = "'" + str(batchEffectiveDatetime) + "'",
                                   ...
                                   string_filter_scmextdlossdtl = str(string_filter_scmextdlossdtl),
                                   string_filter_scmextdpoldtl = str(string_filter_scmextdpoldtl))
