...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Reinstatement_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Reinstatement table in conformed zone</td></tr>
...
sqlQuery= """
SELECT 
    CONCAT(rnst.polid, '_', rnst.unitpsu)                              AS Policy_Section_Reference,
    CONCAT(rnst.polid, '_', rnst.unitpsu)                              AS Policy_Header_reference, 
    COALESCE(rnst.seqno, 0)                                            AS Reinstatement_Number,       
    CAST(COALESCE(rnst.pctg, 0.00) AS DECIMAL(19,8))                   AS Reinstatement_Percentage,
    CONCAT(rnst.polid, '_', rnst.unitpsu)                              AS Coverage_Reference,
    CASE 
        WHEN upper(rnst.unlmt) IN ('TRUE', '1', 'Y' ) THEN 'Y' 
        ELSE 'N'  
    END                                                                AS Unlimited_Indicator,
    GREATEST(rnst.lakelastupdatedate, main.lakelastupdatedate, 
    ip.lakelastupdatedate)                                             AS LakeLastUpdateDate,
    GREATEST(rnst.lakelastupdatetimestamp, main.lakelastupdatetimestamp, 
    ip.lakelastupdatetimestamp)                                        AS LakeLastUpdateTimestamp,
    CASE 
        WHEN COALESCE(rnst.lakeDeletedTimestamp, main.lakeDeletedTimestamp, ip.lakeDeletedTimestamp) IS NULL THEN NULL 
        ELSE {batch_effective_datetime} 
    END                                                                AS lakeDeletedTimestamp
FROM   standardised_subscribe.dbo_inpolrnst rnst

-- to fetch the policies that exists in main table
INNER JOIN standardised_subscribe.dbo_polmain main
ON UPPER(rnst.polid) = UPPER(main.polid)
AND UPPER(rnst.unitpsu) = UPPER(main.unitpsu)
AND {batch_effective_datetime} >= rnst.LakeValidFromTimestamp 
AND {batch_effective_datetime} < COALESCE(rnst.LakeValidToTimestamp, CURRENT_TIMESTAMP())
...
ON UPPER(rnst.polid) = UPPER(ip.polid)
AND UPPER(rnst.unitpsu) = UPPER(ip.unitpsu)
AND {batch_effective_datetime} >= rnst.LakeValidFromTimestamp 
AND {batch_effective_datetime} < COALESCE(rnst.LakeValidToTimestamp, CURRENT_TIMESTAMP())
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwReinstatement'
  viewTables = 'standardised_Subscribe.dbo_PolMain, standardised_subscribe.dbo_inpolrnst, standardised_subscribe.dbo_inpol'
