...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_mdm_Loss_Event_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Loss Event table in conformed zone</td></tr>
# MAGIC   <tr><td><b>Author</b></td><td>Rahul</td></tr>
# MAGIC   <tr><td><b>Creation Date</b></td><td>2022/06/23</td></tr>
# MAGIC </table>
# MAGIC 
# MAGIC 
# MAGIC <table>
...
sqlQuery = """
SELECT 
    COALESCE(leh.loss_code, {missing_mdm})                                                  AS Loss_Code,
    COALESCE(le.peril_code, {missing_mdm})                                                  AS Peril_Code,
    COALESCE(lse.location_sub_region_code, {missing_mdm})                                   AS Location_Sub_Region_Code,
    COALESCE(lse.location_country_code, {missing_mdm})                                      AS Country_Alpha_3_Code,
    COALESCE(lse.location_sub_division_code, {missing_mdm})                                 AS Location_Sub_Division_Code,
    COALESCE(leh.loss_code_type, {missing_mdm})                                             AS Loss_Code_Type,
    COALESCE(lse.loss_sub_event_code, {missing_mdm})                                        AS Loss_Sub_Event_Code,
    COALESCE(lse.loss_sub_event_name, {missing_mdm})                                        AS Loss_Sub_Event_Name,
    COALESCE(lse.loss_sub_event_description, {missing_mdm})                                 AS Loss_Sub_Event_Description,
    COALESCE(lse.loss_sub_event_date_from, {missing_startdate})                             AS Loss_Sub_Event_Date_From,
    COALESCE(lse.loss_sub_event_date_to, {missing_enddate})                                 AS Loss_Sub_Event_Date_To,
    COALESCE(le.loss_event_code, {missing_mdm})                                             AS Loss_Event_Code,
    COALESCE(le.loss_event_name, {missing_mdm})                                             AS Loss_Event_Name,
    COALESCE(le.loss_event_description, {missing_mdm})                                      AS Loss_Event_Description,
    COALESCE(le.loss_event_date_from, {missing_startdate})                                  AS Loss_Event_Date_From,
    COALESCE(le.loss_event_date_to, {missing_enddate})                                      AS Loss_Event_Date_To,
    COALESCE(le.attrition_loss_event_override, {missing_mdm})                               AS Attrition_Loss_Event_Override,
    COALESCE(le.large_loss_event_override, {missing_mdm})                                   AS Large_Loss_Event_Override,
    COALESCE(le.catastrophe_loss_event_override, {missing_mdm})                             AS Catastrophe_Loss_Event_Override,
    COALESCE(le.aggregatable_loss_event, {missing_mdm})                                     AS Aggregatable_Loss_Event,
    COALESCE(le.reserving_loss_event, {missing_mdm})                                        AS Reserving_Loss_Event,
    COALESCE(le.modelled_catastrophe, {missing_mdm})                                        AS Modelled_Catastrophe,
    COALESCE(le.loss_event_report_group, {missing_mdm})                                     AS Loss_Event_Report_Group,
    COALESCE(le.Loss_Event_Catastrophe_Model_Code, {missing_mdm})                           AS Loss_Event_Catastrophe_Model,
    Greatest(leh.lakelastupdatedate, lse.lakelastupdatedate, 
       le.lakelastupdatedate)                                                              AS lakelastupdatedate,
    Greatest(leh.lakelastupdatetimestamp, lse.lakelastupdatetimestamp, 
       le.lakelastupdatetimestamp)                                                         AS lakelastupdatetimestamp,
    CASE
        WHEN leh.lakeDeletedTimestamp IS NULL THEN NULL ELSE {batch_effective_datetime}
...
LEFT JOIN 
(
    SELECT location_sub_region_code,
           location_sub_division_code,
           loss_sub_event_code,
           loss_sub_event_name,
           loss_sub_event_description,
           loss_sub_event_date_from,
           loss_sub_event_date_to,
           loss_event_code,
           lakelastupdatedate,
           location_country_code,
           lakelastupdatetimestamp
    FROM   standardised_mdm.mdm_loss_sub_event
    WHERE  {batch_effective_datetime} >= lakevalidfromtimestamp
    AND    {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
    AND    lakedeletedtimestamp IS NULL
) lse
ON leh.loss_sub_event_code = lse.loss_sub_event_code
LEFT JOIN
(
    SELECT loss_event_code,
           loss_event_name,
           loss_event_description,
           loss_event_date_from,
           loss_event_date_to,
           attrition_loss_event_override,
           large_loss_event_override,
           catastrophe_loss_event_override,
           aggregatable_loss_event,
           reserving_loss_event,
           modelled_catastrophe,
           loss_event_report_group,
           peril_code,
           code,
           Loss_Event_Catastrophe_Model_Code,
           lakelastupdatedate,
           lakelastupdatetimestamp
    FROM   standardised_mdm.mdm_loss_event
    WHERE  {batch_effective_datetime} >= lakevalidfromtimestamp
    AND    {batch_effective_datetime} < COALESCE(lakevalidtotimestamp, CURRENT_TIMESTAMP())
    AND    lakedeletedtimestamp IS NULL
) le
ON lse.loss_event_code = le.code
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwLoss_Event'
  viewTables = 'standardised_mdm.mdm_loss_event, standardised_mdm.mdm_loss_event_header, standardised_mdm.mdm_loss_sub_event'
...
try:
  if dependentTableIsUpdated == True:
    ...
    sqlQuery = sqlQuery.format(batch_effective_datetime = "'" + str(batchEffectiveDatetime) + "'",
                               ...
                               missing_enddate = "'" + str(missing_enddate) +"'",
