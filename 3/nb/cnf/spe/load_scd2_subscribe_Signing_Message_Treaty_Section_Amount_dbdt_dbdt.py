...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Signing_Treaty_Section_Amount_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Signing_Treaty_Section_Amount table in conformed zone</td></tr>
...
COALESCE(NULLIF(Trim(tty.ttysect), ''), {missing_string})           AS Signing_Message_Treaty_Section_Code,
tty.cac                                                             AS Lloyds_Central_Accounting_Category_Code,
CAST(tty.netamt AS DECIMAL(28,2))                                   AS Line_SCC,
tty.qulgcgycd                                                       AS Qualification_Category,
greatest(tty.lakelastupdatedate, usm.lakelastupdatedate)            AS lakelastupdatedate,
greatest(tty.lakelastupdatetimestamp, usm.lakelastupdatetimestamp)  AS lakelastupdatetimestamp,
CASE 
    WHEN coalesce(tty.lakeDeletedTimestamp, usm.lakeDeletedTimestamp) IS NULL
    THEN NULL ELSE {batch_effective_datetime} 
END                                                                 AS lakeDeletedTimestamp
FROM standardised_subscribe.dbo_usmtty tty

-- to fetch the transactions id's that exists in UsmMain and get the policies
INNER JOIN standardised_subscribe.dbo_usmmain usm
ON  upper(tty.trnid) = upper(usm.trnid)
AND  upper(tty.trncgy) = upper(usm.trncgy)
AND {batch_effective_datetime} >= tty.lakevalidfromtimestamp 
AND {batch_effective_datetime} < COALESCE(tty.lakevalidtotimestamp, CURRENT_TIMESTAMP())
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwSigning_Message_Treaty_Section_Amount'
  viewTables = 'standardised_subscribe.dbo_usmtty,standardised_subscribe.dbo_usmmain'
