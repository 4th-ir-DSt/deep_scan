...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Item_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Item table in conformed zone</td></tr>
...
sqlQuery= """
SELECT
Concat(item.polid,'_',item.unitpsu)                                          AS Item_Reference,
Concat(item.polid,'_',item.unitpsu)                                          AS Policy_Header_Reference,
Concat(item.polid,'_',item.unitpsu)                                          AS Policy_Section_Reference,
COALESCE(NULLIF(item.Cd,''), {missing_string})                               AS Source_Item_Type_Code,
'Y'                                                                          AS Main_Item_Indicator,
CASE 
    WHEN it_ref.lakeDeletedTimestamp is not null then {missing_mdm} 
    ELSE COALESCE(NULLIF(Trim(it_ref.Item_Type_Code),''),{missing_mdm}) 
END                                                                          AS Item_Type_Code,  
CASE 
    WHEN it_ref.lakeDeletedTimestamp is not null then {missing_mdm} 
    ELSE COALESCE(NULLIF(Trim(it_ref.Item_Type_Name),''),{missing_mdm}) 
END                                                                          AS Item_Type_Description,
greatest(item.lakelastupdatedate, main.lakelastupdatedate,
    ip.lakelastupdatedate, it_ref.lakelastupdatedate)                        AS lakelastupdatedate,
greatest(item.lakelastupdatetimestamp, main.lakelastupdatetimestamp,
    ip.lakelastupdatetimestamp, it_ref.lakelastupdatetimestamp)              AS lakelastupdatetimestamp,
CASE
    WHEN coalesce(item.lakeDeletedTimestamp, main.lakeDeletedTimestamp, ip.lakeDeletedTimestamp) IS NULL THEN NULL 
    ELSE {batch_effective_datetime} 
END                                                                          AS lakeDeletedTimestamp
FROM     
(
    select * from standardised_subscribe_dbo_polanlycd_tmpvw
    WHERE upper(ty) IN ({string_filter})
...
ON  upper(item.polid) = upper(main.polid)
AND upper(item.unitpsu) = upper(main.unitpsu)
...
ON  upper(item.polid) =  upper(ip.polid)
AND upper(item.unitpsu) =  upper(ip.unitpsu)
...
-- MDS feild: Item Type and Item Description
LEFT JOIN  standardised_mdm.mdm_item_Type_mapping it_ref
ON  item.Cd = it_ref.Source_Item_Type_Code
AND lower(it_ref.Source_Name) = 'subscribe'
AND {batch_effective_datetime} >= it_ref.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(it_ref.lakevalidtotimestamp, CURRENT_TIMESTAMP())
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwItem'
  viewTables = 'standardised_Subscribe.dbo_PolMain, standardised_subscribe.dbo_polanlycd, standardised_subscribe.dbo_inpol, standardised_mdm.mdm_item_Type_mapping'
