...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_Claim_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Claim table in conformed zone</td></tr>
...
# MAGIC   </tr>
# MAGIC     <tr>
# MAGIC     <td>20/01/2023</td>
# MAGIC     <td>Harish N</td>
# MAGIC     <td>Added  createStandardisedGrainView cell to handle the updates from standardiset to go as deletes in conformed when grain changes to lower</td>
# MAGIC     </tr> 
# MAGIC   <tr>
# MAGIC     <td></td>
# MAGIC     <td></td>
...
try:
  import pandas as pd
  import pyodbc
  from pyspark.sql.functions import rank,desc,col,lag, when, lit, concat,row_number,collect_list, coalesce, concat_ws,approx_count_distinct,first,last,rank,max,lead,upper,trim,size,collect_set,count
...
coalesce(nullif(trim(main.UCR), ''), {missing_string})                           AS Unique_Claim_Reference,
max(Greatest(movt.Lakelastupdatedate, main.Lakelastupdatedate))                  AS lakelastupdatedate,
max(Greatest(movt.Lakelastupdatetimestamp, main.Lakelastupdatetimestamp))        AS lakelastupdatetimestamp,
CASE 
    WHEN max(coalesce(movt.lakeDeletedTimestamp, main.lakeDeletedTimestamp)) IS NOT NULL THEN {batch_effective_datetime} ELSE NULL 
...
GROUP BY coalesce(upper(nullif(trim(main.BPR), '')), {missing_string}), 
         coalesce(nullif(trim(main.UCR), ''), {missing_string})
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwClaim'
  viewTables = 'standardised_subscribe.dbo_scmmovt, standardised_subscribe.dbo_scmmain'
...
try:
    ...
    union_df.createOrReplaceTempView('standardised_subscribe_dbo_scmmovt_tmpvw')
...
# DBTITLE 1,Create standardised Grain View
try:
    std_object = 'standardised_subscribe.dbo_scmmain'
    current_bed = batchEffectiveDatetime
    chk_column_list = ['BPR']
    ...
    union_df.createOrReplaceTempView('standardised_subscribe_dbo_scmmain_tmpvw')
