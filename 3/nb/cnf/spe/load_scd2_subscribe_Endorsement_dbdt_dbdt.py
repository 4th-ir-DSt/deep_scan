...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>load_subscribe_endorsement_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data for Endorsement table in conformed zone</td></tr>
...
sqlQuery="""
SELECT
endrsmnt_subs_attr.Policy_Section_Reference,
endrsmnt_subs_attr.Policy_Header_Reference,
endrsmnt_subs_attr.Endorsement_Reference,
endrsmnt_subs_attr.Endorsement_Type_Code,
CASE 
    WHEN endrsmnt_typ_mds.lakedeletedtimestamp is not null THEN {missing_mdm} 
    ELSE coalesce(endrsmnt_typ_mds.Endorsement_Type_Name,{missing_mdm}) 
END                                                                                            AS Endorsement_Type_Name,
endrsmnt_subs_attr.Endorsement_Description,
endrsmnt_subs_attr.Endorsement_Effective_From_Date,
endrsmnt_subs_attr.Endorsement_Effective_To_Date,
endrsmnt_subs_attr.Endorsed_By,
greatest(endrsmnt_subs_attr.lakelastupdatedate, endrsmnt_typ_mds.lakelastupdatedate)           AS lakelastupdatedate,
greatest(endrsmnt_subs_attr.lakelastupdatetimestamp, endrsmnt_typ_mds.lakelastupdatetimestamp) AS lakelastupdatetimestamp,
CASE 
    WHEN endrsmnt_subs_attr.lakeDeletedTimestamp IS NULL THEN NULL ELSE {batch_effective_datetime} 
END                                                                                            AS lakeDeletedTimestamp
FROM
(
    SELECT 
    COALESCE(Concat_ws('_', polpm.pmdsc, polpm.settpmamt),{missing_string})                    AS Endorsement_Description,
    concat_ws('_',polpm.polid, polpm.unitpsu, polpm.pmlineno)                                  AS Endorsement_Reference,
    coalesce(polpm.eftvfr, {missing_startdate_string})                                         AS Endorsement_Effective_From_Date,
    coalesce(polpm.eftvto, {missing_enddate_string})                                           AS Endorsement_Effective_To_Date,
    polpm.usrpsu                                                                               AS Endorsed_By,
    Concat(polpm.polid, '_', polpm.unitpsu)                                                    AS Policy_Header_Reference,
    Concat(polpm.polid, '_', polpm.unitpsu)                                                    AS Policy_Section_Reference,
    CASE 
        WHEN lower(pmdsc) like '%canc%' and cncldt=incpdt and upper(entty) IN ('RP') THEN 'CWI'
        WHEN lower(pmdsc) like '%canc%' and upper(entty) IN ('RP')                   THEN 'CAN'
        WHEN lower(pmdsc) like '%corr%'                                              THEN 'COR'
        WHEN (lower(pmdsc) like '%ext%' and lower(pmdsc) NOT LIKE '%next%' and lower(pmdsc) NOT LIKE '%extra%') 
              OR (lower(pmdsc) like '%days on cover%')                               THEN 'EXT'
        WHEN lower(pmdsc) like '%cover%'                                             THEN 'CVG'
        WHEN upper(entty)='AJ'                                                       THEN 'ADJ' 
        ELSE {missing_string}
    END                                                                                        AS Endorsement_Type_Code,
    greatest(polpm.lakelastupdatedate, main.lakelastupdatedate,
    ip.lakelastupdatedate)                                                                     AS lakelastupdatedate,
    greatest(polpm.lakelastupdatetimestamp, main.lakelastupdatetimestamp,
    ip.lakelastupdatetimestamp)                                                                AS lakelastupdatetimestamp,
    CASE 
        WHEN coalesce(polpm.lakeDeletedTimestamp, main.lakeDeletedTimestamp, ip.lakeDeletedTimestamp) IS NULL
    ...
    (
        ...
        WHERE ({queryWhereCondition_polpm})
    ...
    --to fetch only those policies from Premium that exists in Main table
    INNER JOIN standardised_subscribe.dbo_polmain main
    ON upper(polpm.polid) = upper(main.polid)
    AND upper(polpm.unitpsu) = upper(main.unitpsu)
    ...
    ON  upper(polpm.polid) =  upper(ip.polid)
    AND    upper(polpm.unitpsu) =  upper(ip.unitpsu)
...
--to fetch Endorsement Type
LEFT JOIN standardised_mdm.mdm_endorsement_type endrsmnt_typ_mds
ON endrsmnt_subs_attr.Endorsement_Type_Code = endrsmnt_typ_mds.Endorsement_Type_Code
AND {batch_effective_datetime} >= endrsmnt_typ_mds.lakevalidfromtimestamp
AND {batch_effective_datetime} < COALESCE(endrsmnt_typ_mds.lakevalidtotimestamp, CURRENT_TIMESTAMP())
"""
...
try:
  ...
  viewName = 'dummy_Dummy.dummyVwEndorsement'
  viewTables = 'standardised_Subscribe.dbo_PolMain,standardised_Subscribe.dbo_polpm, standardised_subscribe.dbo_inpol, standardised_mdm.mdm_endorsement_type'
...
try:
    ...
    pmseqno <> 1                         --Assume first movement is the original agreed premium
    AND upper(entty) IN ('AP','RP','AJ') --Assume subsequent movements to the original premium are corrections, not endorsements
    AND upper(pmst) = 'ORIG'             --Only use Premium Status of "Original". Ignore the Cancelled/Contras
    """
