...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_get_data_quality_test</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>unit test stored procedure usp_get_data_quality_test</td></tr>
...
try :
  ...
  testObjectName = 'uspGetDataQualityTest'  
...
#run cleanup script
try:
  cleanup = open("{}UspGetDataQualityTestCleanUp1.sql".format(testInputPath), "r").read()
...
try:
  ...
  inputParameter = open('{}UspGetDataQualityTestDataPrep1.sql'.format(testInputPath), "r").read()
...
# DBTITLE 1,Scenario-1 Verify stored procedure output when data quality test is active
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify stored procedure output when data quality test is active'
  #Execute stored procedure
  spOutPut = pd.read_sql_query("exec [config].[usp_get_data_quality_test] {}".format(newBatchTaskId), conn) 
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_get_data_quality_test')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 1 stored procedure usp_get_data_quality_test : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  #run cleanup script
  cleanupMetadata = open("{}UspGetDataQualityTestCleanUp1.sql".format(testInputPath), "r").read()
...
try:
  #inserting data into tbl_task and tbl_batch_task
  inputParameter = open('{}UspGetDataQualityTestDataPrep2.sql'.format(testInputPath), "r").read()
  #execute the prep1 script and get the first row
  inputParameterResults = pd.read_sql_query(inputParameter,conn).iloc[0]
...
# DBTITLE 1,Scenario-2 Verify stored procedure output when data quality test is inactive
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify stored procedure output when data quality test is inactive'
  #Execute stored procedure
  spOutPut = pd.read_sql_query("exec [config].[usp_get_data_quality_test] {}".format(newBatchTaskId), conn) 
  #Verify record inserted through stored procedure, If shape is 1 then testcase is success 
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 2 stored procedure usp_get_data_quality_test')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 2 stored procedure usp_get_data_quality_test : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
