...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_log_unit_test</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>unit test stored procedure usp_log_unit_test</td></tr>
...
try :
  ...
  testObjectName = 'uspLogUnitTest'  
  requiredInputParameter = 'testObject,testObjectName,requiredInputParameter,testCaseScenario,executionOutputStatus,                                                 sampleOutputLocation,executionDatetime,testCaseStatus'
  testCaseScenario = ''
...
#run cleanup script
try:
  cleanup = open("{}LogUnitTestCleanUp.sql".format(testInputPath), "r").read()
...
try:
  pTestObject = 'stored procedure'
  pTestObjectName = 'testUspLogUnitTest'
  pRequiredInputParameter = 'Parameters to test stored procedure'
  pTestCaseScenario = 'Test stored procedure'
  pExecutionOutputStatus = 'As expected'
  pSampleOutputLocation = 'NA'
  pExecutionDatetime = datetime.now()
  pTestCaseStatus = 'success'
...
# DBTITLE 1,Scenario-1 Verify stored procedure is logging status in tbl_unit_test_results
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify stored procedure is logging status in tbl_unit_test_results'
  #Execute stored procedure
  cursor.execute("exec [audit].[usp_log_unit_test]?,?,?,?,?,?,?,?",pTestObject, pTestObjectName, pRequiredInputParameter,                                  pTestCaseScenario, pExecutionOutputStatus, pSampleOutputLocation, pExecutionDatetime, pTestCaseStatus)
  while cursor.nextset():
    x = 1
  #Get records for the inserted test_object_name  
  spInsertedResult = pd.read_sql_query("SELECT * FROM audit.tbl_unit_test_result WHERE test_object_name = '{}'".format(pTestObjectName), conn)  
  #Verify record inserted through stored procedure, If shape is 1 then testcase is success 
  if spInsertedResult.shape[0] == 1:
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_log_unit_test')  
except Exception as e:
  errorMessage = "Exception occured while testing scenario 1 stored procedure usp_log_unit_test : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False

# COMMAND ----------

# DBTITLE 1,Scenario-2 Negative test case with missing parameters 
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  ...
  try:
    cursor.execute("exec [audit].[usp_log_unit_test]?,?,?,?,?,?,?,?",pTestObject, pTestObjectName, pRequiredInputParameter,                                  pTestCaseScenario, pExecutionOutputStatus, pSampleOutputLocation, pExecutionDatetime)
    while cursor.nextset():
      x = 1
  except Exception as e:
    executionOutputStatus = 'As expected'
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 2 stored procedure usp_log_unit_test')  
except Exception as e:
  errorMessage = "Exception occured while testing scenario 2 stored procedure usp_log_unit_test : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  #run cleanup script
  cleanupMetadata = open("{}LogUnitTestCleanUp.sql".format(testInputPath), "r").read()
