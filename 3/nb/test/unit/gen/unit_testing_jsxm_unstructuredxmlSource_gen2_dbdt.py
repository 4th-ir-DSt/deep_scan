...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_jsxm_unstructuredxmlSource_gen2_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Unit test for jsxm_unstructuredxmlSource_gen2_dbdt </td></tr>
...
# MAGIC   <td>Updated the markDatesLoadedFunction</td>
# MAGIC </table>
# MAGIC 
...
try :
  ...
  outputLocation = ''
  testObject = 'notebook'
  testObjectName = 'jsxmunstructuredxmlSourceGen2dbdt'
...
try: 
  metadatacleanup = open("{}jsxmunstructuredxmlSourceGen2DbdtHighAndLowLevelCleanUp.sql".format(testInputPath), "r").read()
...
try:
  ...
  inputParmeter = open("{}jsxmunstructuredxmlSourceGen2DbdtHighAndLowLevelPrep1.sql".format(testInputPath), "r").read()
  ...
  dataPreparation = open("{}jsxmunstructuredxmlSourceGen2DbdtHighAndLowLevelPrep2.sql".format(testInputPath), "r").read()
...
try:
  ...
  #call spExecHighAndLowLevel function
  ...
  objectName = 'unitTestJsxmunstructuredxmlSourceGen2Dbdt_a'
...
try :
  ...
  expectedResults = [('ROW_ID', 'row_id AS ROW_ID', 'none', 'row_id', 'STRING', 'STRING')]
  ...
  if len(ColumnListFinal) >= 1 and ColumnListFinal == expectedResults: #If test result is true, make test result is success
    testResult = 'success'
    executionOutputStatus = 'As Expected'
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS unitTestJsxmunstructuredxmlSourceGen2Dbdt_a

# COMMAND ----------

# DBTITLE 1,Create input Delta table
# MAGIC %sql
# MAGIC create table unitTestJsxmunstructuredxmlSourceGen2Dbdt_a(
# MAGIC row_id int,
# MAGIC country string)
# MAGIC using DELTA
# MAGIC Location "/mnt/dataquality/unit_tests/jsxmunstructuredxmlSourceGen2dbdt/input/unitTestJsxmunstructuredxmlSourceGen2Dbdt_a"

# COMMAND ----------

# DBTITLE 1,Insert data into source table
try:
  fileName = "jsxmunstructuredxmlSourceGen2Dbdt.csv"
  inputFormat = "delta"
  srcFile = tableInputPath + fileName
  tgtTableLocation = tableInputPath + 'unitTestJsxmunstructuredxmlSourceGen2Dbdt_a'
  schema = StructType(
...
try :
  ...
  #compare the count 
  if sourceSelectWhereDF.count() > 0:
    testResult = 'success'
    executionOutputStatus = 'As Expected'
...
try:
  ...
  requiredInputParameter = 'destinationObjectId, datetimeFrom, datetimeTo, cursor,batchTaskId,adfPipelineName,clusterId,notebookName,errorLogFileLocation'
  testResult = 'failed' #initialise the test result
  executionOutputStatus = 'Mismatched'#initialise the execution_output_status
  
  #assign datetime
  newDatetimeFrom = newDatetimeTo = newOutputDateTime
  
  #call markDatesLoaded function
  
  conn, cursor = markDatesLoaded(newDestinationObjectId
                                ,newSourceObjectId
                                ,newDatetimeFrom
                                ,newDatetimeTo
                                ,dbconn
