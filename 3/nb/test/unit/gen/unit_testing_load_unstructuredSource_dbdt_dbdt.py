...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>Unit test for unit_testing_load_unstructuredSource_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Unit test for load_unstructuredSource_dbdt_dbdt notebook </td></tr>
...
# MAGIC     <td>Updated MarkDatesLoaded Function with two new parameters</td>
# MAGIC   </tr>
# MAGIC </table>
# MAGIC 
...
try :
  ...
  outputLocation = ''
  testObject = 'notebook'
  testObjectName = 'loadunstructuredSourceDbdtDbdt'
...
# MAGIC %sql
# MAGIC drop table if exists testLoadunstructuredSourceDbdtDbdt_a;
# MAGIC drop table if exists Stg_testLoadunstructuredSourceDbdtDbdt_b;
# MAGIC drop table if exists Ref_testLoadunstructuredSourceDbdtDbdt_c;
# MAGIC drop table if exists Ref_testLoadunstructuredSourceDbdtDbdt_d;

# COMMAND ----------

# DBTITLE 1,Remove the directory for previous runs
dbutils.fs.rm(tableInputPath + '/testLoadunstructuredSourceDbdtDbdt_a',recurse=True)
dbutils.fs.rm(tableInputPath + '/Stg_testLoadunstructuredSourceDbdtDbdt_b',recurse=True)
dbutils.fs.rm(tableInputPath + '/Ref_testLoadunstructuredSourceDbdtDbdt_c',recurse=True)
dbutils.fs.rm(tableInputPath + '/Ref_testLoadunstructuredSourceDbdtDbdt_d',recurse=True)

# COMMAND ----------

# DBTITLE 1,Create source delta table
# MAGIC %sql
# MAGIC create table testLoadunstructuredSourceDbdtDbdt_a(
# MAGIC row_id string,
# MAGIC id     string)
# MAGIC using DELTA
# MAGIC Location "/mnt/dataquality/unit_tests/loadunstructuredSourceDbdtDbdt/input/testLoadunstructuredSourceDbdtDbdt_a";
# MAGIC 
# MAGIC -- insert data into table
# MAGIC insert into testLoadunstructuredSourceDbdtDbdt_a  values('aaa','a');
# MAGIC insert into testLoadunstructuredSourceDbdtDbdt_a  values('bbb','b');
# MAGIC insert into testLoadunstructuredSourceDbdtDbdt_a  values('ccc','c');
# MAGIC 
# MAGIC create table Ref_testLoadunstructuredSourceDbdtDbdt_c(
# MAGIC row_id string,
# MAGIC LastName   string)
# MAGIC using DELTA
# MAGIC Location "/mnt/dataquality/unit_tests/loadunstructuredSourceDbdtDbdt/input/Ref_testLoadunstructuredSourceDbdtDbdt_c";
# MAGIC 
# MAGIC -- insert data into table
# MAGIC insert into Ref_testLoadunstructuredSourceDbdtDbdt_c  values('aaa','x');
# MAGIC insert into Ref_testLoadunstructuredSourceDbdtDbdt_c  values('bbb','y');
# MAGIC insert into Ref_testLoadunstructuredSourceDbdtDbdt_c  values('ccc','z');
# MAGIC 
# MAGIC create table Ref_testLoadunstructuredSourceDbdtDbdt_d(
# MAGIC row_id string,
# MAGIC FirstName     string)
# MAGIC using DELTA
# MAGIC Location "/mnt/dataquality/unit_tests/loadunstructuredSourceDbdtDbdt/input/Ref_testLoadunstructuredSourceDbdtDbdt_d";
# MAGIC 
# MAGIC -- insert data into table
# MAGIC insert into Ref_testLoadunstructuredSourceDbdtDbdt_d  values('aaa','xxx');
# MAGIC insert into Ref_testLoadunstructuredSourceDbdtDbdt_d  values('bbb','yyy');
# MAGIC insert into Ref_testLoadunstructuredSourceDbdtDbdt_d  values('ccc','zzz');
...
try:
  #run cleanup script
  cleanupMetadata = open("{}LoadunstructuredSourceDbdtDbdtCleanUp.sql".format(testInputPath), "r").read()
...
try:
  ...
  inputParmeter = open("{}LoadunstructuredSourceDbdtDbdtDataPrep1.sql".format(testInputPath), "r").read()
  ...
  #insertion of data into object,task_object_map, object_definition and object_dates_availability tables
  dataPreparation = open("{}LoadunstructuredSourceDbdtDbdtDataPrep2.sql".format(testInputPath), "r").read()
...
try:
  ...
  if len(joinFieldsList) > 0 :
    
    #Get destination dataframe with join columns 
    joinSourceDf = getJoinSourceDf(sourceTableName
                                   ,whereExpression
                                   ,joinFieldsList
                                   ,jsonStartString
                                   ,cursor
    ...
    #Pass destination dataframe to selectDestFromJoinSrc function
    sourceSelectWhereDF = selectDestFromJoinSrc(joinSourceDf
  ...
  #If there is no join type column run function 'selectDestFromSrc'
  else:
...
# MAGIC %sql
# MAGIC create table Stg_testLoadunstructuredSourceDbdtDbdt_b(
# MAGIC ROW_ID string,
# MAGIC FirstName     string,
# MAGIC LAstName string)
# MAGIC using DELTA
# MAGIC Location "/mnt/dataquality/unit_tests/loadunstructuredSourceDbdtDbdt/input/Stg_testLoadunstructuredSourceDbdtDbdt_b";
...
try:
  ...
  destinationTable = 'Stg_testLoadunstructuredSourceDbdtDbdt_b' 
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on successfully for scenario 1')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 1 function: " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,outputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on successfully for scenario 2')  
except Exception as e:
...
# DBTITLE 1,Scenario-3 Verify is destination is taking columns from reference table-1
#While preparing metadata for reference table-1 column name is 'firstname' and for row_id 'aaa' value is 'xxx'.So,while loading into destimation table firstname column has to be there in destination and value should be 'xxx'
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify is destination is taking columns from reference table-1' 
  destinationTable = 'Stg_testLoadunstructuredSourceDbdtDbdt_b' 
  funcOutput = spark.sql("select distinct(firstname) from {}".format(destinationTable))
  
  if funcOutput.count() == 1 and  funcOutput.collect()[0][0] == 'xxx':
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'
  #function call to insert a row into tbl_unit_test on the test case status  
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,outputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed successfully for scenario 3')  
except Exception as e:
...
# DBTITLE 1,Scenario-4 Verify is destination is taking columns from reference table-2
#While preparing metadata for reference table-1 column name is 'firstname' and for row_id 'aaa' value is 'x'.So,while loading into destimation table firstname column has to be there in destination and value should be 'x'
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = ' Verify is destination is taking columns from reference table-2' 
  destinationTable = 'Stg_testLoadunstructuredSourceDbdtDbdt_b' 
  funcOutput = spark.sql("select distinct(lastname) from {}".format(destinationTable))
  
  if funcOutput.count() == 1 and  funcOutput.collect()[0][0] == 'x':
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'
  #function call to insert a row into tbl_unit_test on the test case status  
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,outputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed successfully for scenario 4 ')  
except Exception as e:
...
try:
  #execute the script and get batch_task_id and task_id
  inputParmeter = open("{}LoadunstructuredSourceDbdtDbdtDataPrep3.sql".format(testInputPath), "r").read()
...
try:
  #call the markDatesLoaded Function
...
# DBTITLE 1,Scenario-5 Verify function markDatesLoaded is updating
