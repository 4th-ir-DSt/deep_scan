...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_load_na_gen2_sqdb</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>unit test notebook load_na_gen2_sqdb</td></tr>
...
# MAGIC       <br>modified object_id to output_object_id in test cases
...
def testDataLoadPreparation():
  try:
    sampleFile = '/mnt/dataquality/unit_tests/loadNaGen2Sqdb/input/sampleTestAvroFile.avro'
    dateTestPath = '/mnt/raw/testLoadNaGen2Sqdb/eh_testLoadNaGen2Sqdb/input/dateTestFiles/' 
    dateTimeTestPath = '/mnt/raw/testLoadNaGen2Sqdb/eh_testLoadNaGen2Sqdb/input/dateTimeTestFiles/' 
    ...
    #To test if file is in second date direcotory
    ...
    logTaskProgress(cursor,batchTaskId,'Successfully copied files into directories to test load_na_gen2_sqdb')

  except Exception as e:
    errorMessage = 'Exception occurred while copying files into directories to test load_na_gen2_sqdb ' + str(e)
...
try:
  ...
  dbutils.fs.rm('/mnt/raw/testLoadNaGen2Sqdb',recurse = True)
  #cleanup the previous inserted data
  cleanup = open("{}LoadNaGen2SqdbCleanUp.sql".format(testInputPath), "r").read()
...
try:
  ...
  mainDirTime = ["/mnt/raw/testLoadNaGen2Sqdb/eh_testLoadNaGen2Sqdb/input/dateTimeTestFiles"] 
  ...
  mainDirDate = ["/mnt/raw/testLoadNaGen2Sqdb/eh_testLoadNaGen2Sqdb/input/dateTestFiles"] 
...
# DBTITLE 1,Schema for input data
try:
  ...
  errorMessage = "Exception occurred while preparing schema for test " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 

# COMMAND ----------

# DBTITLE 1,Get input data from stored procedure to test scenarios
#Get object from stored procedure to pass to function
try:
  ...
  testScenariosDf = spOutput.selectExpr('*').where(spOutput['location'].like('/mnt/raw/testLoadNaGen2Sqdb/eh_testLoadNaGen2Sqdb/input/%'))
  logTaskProgress(cursor,batchTaskId,'Successfully excuted stored procedure to get input data')
except Exception as e:
  errorMessage = "Exception occurred while getting input data from stored procedure " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False   
...
try:
  ...
  logTaskProgress(cursor,batchTaskId,'Successfully executed function rawObjectLatestDateTime')
except Exception as e:
  errorMessage = "Exception occurred while executing function rawObjectLatestDateTime" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
try:
  ...
  testObjectScenario = 'testLoadNaGen2Sqdb_dateTestFiles_hasFilesInFirstDate'
...
try:
  ...
  #Test object while preparing data data for scenario
  testObjectScenario = 'testLoadNaGen2Sqdb_dateTestFiles_hasFilesInSecondDate'
  #Get object_id for testObject
  testObjectId = spark.createDataFrame(pd.read_sql_query("select object_id from config.tbl_object where object_name = '{}'".format(testObjectScenario),conn)).collect()[0][0]
  #Get the datetime from the table tbl_object_dates_availability, which is inserted by function
  dateFromFunction = spark.createDataFrame(pd.read_sql_query("select output_datetime_to from audit.tbl_object_dates_availability where output_object_id = '{}'".format(testObjectId),conn)).collect()[0][0]
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario-2 notebook load_na_gen2_sqdb')
except Exception as e:
  errorMessage = "Exception occurred while prepared data for test scenario-2 notebook load_na_gen2_sqdb" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  

# COMMAND ----------

# DBTITLE 1,Scenario-3: Verify default timestamp is returned when no files are in date directories 
#If there is no files in all date directories it has to return default value i.e., '9999-01-01 00:00:00.000'
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify default timestamp is returned when no files are in date directories'
  #Test object while preparing data data for scenario
  testObjectScenario = 'testLoadNaGen2Sqdb_dateTestFiles_hasNoFiles'
  #Get object_id for testObject
  testObjectId = spark.createDataFrame(pd.read_sql_query("select object_id from config.tbl_object where object_name = '{}'".format(testObjectScenario),conn)).collect()[0][0]
  #Get the datetime from the table tbl_object_dates_availability, which is inserted by function
  dateFromFunction = spark.createDataFrame(pd.read_sql_query("select output_datetime_to from audit.tbl_object_dates_availability where output_object_id = '{}'".format(testObjectId),conn)).collect()[0][0]
  if str(dateFromFunction) == '9999-01-01 00:00:00':
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario-3 notebook load_na_gen2_sqdb')
except Exception as e:
  errorMessage = "Exception occurred while prepared data for test scenario-3 notebook load_na_gen2_sqdb" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  
...
try:
  ...
  #Test object while preparing data data for scenario
  testObjectScenario = 'testLoadNaGen2Sqdb_dateTimeTestFiles_hasFilesInFirstDateAndFirstTime'
  #Get object_id for testObject
  testObjectId = spark.createDataFrame(pd.read_sql_query("select object_id from config.tbl_object where object_name = '{}'".format(testObjectScenario),conn)).collect()[0][0]
  #Get the datetime from the table tbl_object_dates_availability, which is inserted by function
  dateFromFunction = spark.createDataFrame(pd.read_sql_query("select output_datetime_to from audit.tbl_object_dates_availability where output_object_id = '{}'".format(testObjectId),conn)).collect()[0][0]
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario-4 notebook load_na_gen2_sqdb')
except Exception as e:
  errorMessage = "Exception occurred while prepared data for test scenario-4 notebook load_na_gen2_sqdb" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  
...
try:
  ...
  #Test object while preparing data data for scenario
  testObjectScenario = 'testLoadNaGen2Sqdb_dateTimeTestFiles_hasFilesInFirstDateAndSecondTime'
  #Get object_id for testObject
  testObjectId = spark.createDataFrame(pd.read_sql_query("select object_id from config.tbl_object where object_name = '{}'".format(testObjectScenario),conn)).collect()[0][0]
  #Get the datetime from the table tbl_object_dates_availability, which is inserted by function
  dateFromFunction = spark.createDataFrame(pd.read_sql_query("select output_datetime_to from audit.tbl_object_dates_availability where output_object_id = '{}'".format(testObjectId),conn)).collect()[0][0]
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario-5 notebook load_na_gen2_sqdb')
except Exception as e:
  errorMessage = "Exception occurred while prepared data for test scenario-5 notebook load_na_gen2_sqdb" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  
...
try:
  ...
  #Test object while preparing data data for scenario
  testObjectScenario = 'testLoadNaGen2Sqdb_dateTimeTestFiles_hasFilesInSecondDateAndFirstTime'
  #Get object_id for testObject
  testObjectId = spark.createDataFrame(pd.read_sql_query("select object_id from config.tbl_object where object_name = '{}'".format(testObjectScenario),conn)).collect()[0][0]
  #Get the datetime from the table tbl_object_dates_availability, which is inserted by function
  dateFromFunction = spark.createDataFrame(pd.read_sql_query("select output_datetime_to from audit.tbl_object_dates_availability where output_object_id = '{}'".format(testObjectId),conn)).collect()[0][0]
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario-6 notebook load_na_gen2_sqdb')
except Exception as e:
  errorMessage = "Exception occurred while prepared data for test scenario-6 notebook load_na_gen2_sqdb" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  
...
try:
  ...
  #Test object while preparing data data for scenario
  testObjectScenario = 'testLoadNaGen2Sqdb_dateTimeTestFiles_hasFilesInSecondDateAndSecondTime'
  #Get object_id for testObject
  testObjectId = spark.createDataFrame(pd.read_sql_query("select object_id from config.tbl_object where object_name = '{}'".format(testObjectScenario),conn)).collect()[0][0]
  #Get the datetime from the table tbl_object_dates_availability, which is inserted by function
  dateFromFunction = spark.createDataFrame(pd.read_sql_query("select output_datetime_to from audit.tbl_object_dates_availability where output_object_id = '{}'".format(testObjectId),conn)).collect()[0][0]
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario-7 notebook load_na_gen2_sqdb')
except Exception as e:
  errorMessage = "Exception occurred while prepared data for test scenario-7 notebook load_na_gen2_sqdb" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  
...
#If data is not in all date and time level directories it has to return default value. i.e., '9999-01-01 00:00:00'
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  ...
  #Test object while preparing data data for scenario
  testObjectScenario = 'testLoadNaGen2Sqdb_dateTimeTestFiles_hasNoFiles'
  #Get object_id for testObject
  testObjectId = spark.createDataFrame(pd.read_sql_query("select object_id from config.tbl_object where object_name = '{}'".format(testObjectScenario),conn)).collect()[0][0]
  #Get the datetime from the table tbl_object_dates_availability, which is inserted by function
  dateFromFunction = spark.createDataFrame(pd.read_sql_query("select output_datetime_to from audit.tbl_object_dates_availability where output_object_id = '{}'".format(testObjectId),conn)).collect()[0][0]
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario-8 notebook load_na_gen2_sqdb')
except Exception as e:
  errorMessage = "Exception occurred while prepared data for test scenario-8 notebook load_na_gen2_sqdb" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  
...
#If infile_datetime_column has any value it will return timestamp by reading files and it will take from 'EnqueuedTimeUtc' field max value
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  ...
  if str(dateFromFunction) == str(maxDatetime):
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario-9 notebook load_na_gen2_sqdb')
except Exception as e:
  errorMessage = "Exception occurred while prepared data for test scenario-9 notebook load_na_gen2_sqdb" + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  
