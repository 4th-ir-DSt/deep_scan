...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_get_source_types</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Test on stored procedure usp_get_source_types which return source type of the current batch and source</td></tr>
...
try:
  ...
  testObjectName = 'uspGetSourceTypes'
...
#run cleanup script
try:
  cleanup = open("{}getSourceTypeCleanup.sql".format(testInputPath), "r").read()
...
try:  
  inputParameter = open("{}getSourceTypes.sql".format(testInputPath), "r").read()
  inputParameter = inputParameter.replace('<batch_id>',str(batchId))  
  cursor.execute(inputParameter)
  while cursor.nextset():
    x = 1  
  logTaskProgress(cursor,batchTaskId,'Execution of script to generate input parameter for usp_get_source_types')  
except Exception as e:
  errorMessage = "Exception occured while executing script to generate input parameter for usp_get_source_types " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
try:
  ...
  for i in range (0,2):
    try:
      ...
      spOutput = pd.read_sql_query("exec config.usp_get_source_types {}".format(jBatchId[i]),conn)
    except Exception as e:
      ...
      logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 for stored procedure usp_get_source_types') 
except Exception as e:
  errorMessage="Exception occured while testing on scenario 1 for stored procedure usp_get_source_types': " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
try:
  ...
  spOutput = pd.read_sql_query("exec config.usp_get_source_types {}".format(batchId),conn)
  ...
  errorMessage="Exception occured while testing scenario 2 stored procedure usp_get_sources_types : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  ...
  spOutputScenario3 = spOutput[spOutput['source_type_name']=="DataSource"]
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 3 stored procedure usp_get_sources_types')  
except Exception as e:
  errorMessage = "Exception occured while testing scenario 3 stored procedure usp_get_sources_types : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
try:
  cleanup = open("{}getSourceTypeCleanup.sql".format(testInputPath), "r").read()
  cleanup = cleanup.replace('<batch_id>' , str(batchId))
  cursor.execute(cleanup)
  while cursor.nextset():
    x = 1
  logTaskProgress(cursor,batchTaskId,'Successfully executed clean up scripts')   
