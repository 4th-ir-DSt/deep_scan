...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_get_historic_raw_eventhub_object_locations</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>unit test stored procedure usp_get_historic_raw_eventhub_object_locations</td></tr>
...
try :
  ...
  testObjectName = 'uspGetHistoricRawEventHubObjectLocations'  
  baseObjectName = 'TestGetHistoricRawEventHubObjectLocations'
...
#run cleanup script
try:
  cleanup = open("{}GetHistoricRawEventHubObjectLocationsCleanUp.sql".format(testInputPath), "r").read()
...
try:
  insertData = open('{}GetHistoricRawEventHubObjectLocationsDataPrep.sql'.format(testInputPath), "r").read()
...
# DBTITLE 1,Scenario-1 Negative test case by passing inactive object 
#while preparing data only negative object only added in '/mnt/raw/unstructuredxmlSource' folder.So, stored procedure will return no records 
try:
  #Declare variables
  ...
  spOutput = pd.read_sql_query("exec config.usp_get_historic_raw_eventhub_object_locations",conn)  
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_get_historic_raw_eventhub_object_locations')  
except Exception as e:
  errorMessage = "Exception occured while testing scenario 1 stored procedure usp_get_historic_raw_eventhub_object_locations : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  #run cleanup script
  cleanupMetadata = open("{}GetHistoricRawEventHubObjectLocationsCleanUp.sql".format(testInputPath), "r").read()
