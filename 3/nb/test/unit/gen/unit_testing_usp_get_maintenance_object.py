...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>Unit test for unit_testing_usp_get_maintenance_object</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Unit test for usp_get_maintenance_object stored procedure </td></tr>
...
try :
  ...
  testObjectName = 'uspGetMaintenanceObject'
...
try:
  #run cleanup script
  cleanupMetadata = open("{}UspGetMaintenanceObjectCleanUp.sql".format(testInputPath), "r").read()
...
try:
  ...
  inputParameter = open('{}UspGetMaintenanceObjectDataPrep1.sql'.format(testInputPath), "r").read()
...
# DBTITLE 1,Scenario-1 Verify stored procedure returning correct object
# While preparing data objectName is 'TestUspGetMaintenanceObject_a'.If stor proc returns same name and it will return only 1 record then test case is pass 
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify stored procedure returning correct object' 
  objectName = 'TestUspGetMaintenanceObject_a'
  maintenanceObjectsDf = pd.read_sql_query("exec config.usp_get_maintenance_object {}".format(newBatchTaskId), conn)
  dfObjctName = maintenanceObjectsDf.at[0,'object_name']
  #If objectnames matches and count of stor proc is 1 then make test case as success
  if dfObjctName == objectName and maintenanceObjectsDf.shape[0] == 1 :
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_get_maintenance_object')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 1 stored procedure usp_get_maintenance_object : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
