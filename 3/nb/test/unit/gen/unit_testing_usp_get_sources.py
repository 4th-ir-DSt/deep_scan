...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_get_sources</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>unit test on stored procedure usp_get_sources</td></tr>
...
# MAGIC         <br>Dev-rework addition of phase_id in the input parameters</td>
# MAGIC   </tr>
# MAGIC   
# MAGIC   <tr> 
# MAGIC     <td> </td>
...
try:
  ...
  testObjectName = 'uspGetSources'
  requiredInputParameter = 'batchId,sourceTypeId,phaseId'
  
  #path for datasets used in stored procedure testing
...
#run cleanup script
try:
  cleanup = open("{}uspGetSourcesCleanup.sql".format(testInputPath), "r").read()
...
try:  
  inputParameter = open("{}uspGetSourcesInput.sql".format(testInputPath), "r").read()
  ...
  logTaskProgress(cursor,batchTaskId,'Execution of script to generate input parameter for usp_get_sources')  
except Exception as e:
  errorMessage = "Exception occurred while executing script to generate input parameter for usp_get_sources " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
try:
  ...
  jBatchId = ['','l1']
  jSourceTypeId = [234.55,2]
  jPhaseId = [1,'a']
  for i in range (0,2):
    try:
      #run stored procedure for different parameter values which throws error for invaild input values
      spOutput = pd.read_sql_query("exec config.usp_get_sources {},{},{}".format(jBatchId[i],jPhaseId[i],jSourceTypeId[i]),conn)
    except Exception as e:
      executionOutputStatus = 'As expected'
      testCaseStatus = 'success'
      
      specificParameterTestScenarion = testCaseScenario + "which are, batch_id:{} phase_id:{} and source_type_id:{}".format(str(jBatchId[i]),str(jPhaseId[i]),str(jSourceTypeId[i]))
    #function to log the test result in table
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 for stored procedure usp_get_sources') 
except Exception as e:
  errorMessage = "Exception occurred while testing on scenario 1 for stored procedure usp_get_sources': " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False      
...
try:
  ...
  spOutput = pd.read_sql_query("exec config.usp_get_sources {},{},{}".format(batchId,newPhaseId,newSourceTypeId),conn)
  
  #check the count of rows returned by stored procedure
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       ...
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 2 stored procedure usp_get_sources')  
except Exception as e:
  errorMessage="Exception occurred while testing scenario 2 stored procedure usp_get_sources : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  ...
  spOutputSource = spOutput[spOutput['source_name']=="TestingGetSourceProcedureSource"]
  
  #check the row count of filtered dataframe
  if spOutputSource.shape[0] == 1:
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'    
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 3 stored procedure usp_get_sources')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 3 stored procedure usp_get_sources : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  
