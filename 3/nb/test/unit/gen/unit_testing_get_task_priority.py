...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_get_task_priority</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Test on procedure usp_get_task_priority that returns task_priority for the batch</td></tr>
...
try :
  ...
  requiredInputParameter = 'batchId,sourceId'
  testCaseScenario = ''
...
try:  
  ...
  #get the new source id form the return dataset
  ...
  errorMessage = "Exception occured while executing script to generate input parameter for usp_get_task_priorities " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
try:
  ...
  for i in range (0,2):
    try:
      ...
      spOutput = pd.read_sql_query("exec config.usp_get_task_priorties {},{}".format(jBatchId[i],jSourceId[i]),conn)
    except Exception as e:
      executionOutputStatus = 'As expected'
      ...
      logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 for stored procedure usp_get_task_priorties') 
except Exception as e:
  errorMessage="Exception occured while testing on scenario 1 for stored procedure usp_get_task_priorties': " + str(e)
...
try:
  ...
  spOutput = pd.read_sql_query("exec config.usp_get_task_priorties {},{}".format(batchId,newSourceId),conn)
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 2 stored procedure usp_get_task_priorties')  
except Exception as e:
  errorMessage = "Exception occured while testing scenario 2 stored procedure usp_get_task_priorties : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  ...
  spOutputScenario3 = spOutput[spOutput['task_priority']==9]
