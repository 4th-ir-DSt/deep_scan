...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_get_phase_source_types</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Test on stored procedure usp_get_phase_source_types </td></tr>
...
# MAGIC         <br>Renamed stored procedure from get_sources to get_phase_sources,Added phase_id in stored procedure</td>
# MAGIC   </tr>
# MAGIC   <tr>
# MAGIC     <td>  </td>
# MAGIC     <td>  </td>
...
try:
  ...
  testObjectName = 'uspGetPhaseSourceTypes'
...
#run cleanup script
try:
  cleanup = open("{}uspGetPhaseSourceTypesCleanup.sql".format(testInputPath), "r").read()
...
try:  
  inputParameter = open("{}uspGetPhaseSourceTypesInput.sql".format(testInputPath), "r").read()
  inputParameter = inputParameter.replace('<batch_id>',str(batchId))  
  cursor.execute(inputParameter)
  while cursor.nextset():
    x = 1  
  logTaskProgress(cursor,batchTaskId,'Execution of script to generate input parameter for usp_get_phase_source_types')  
except Exception as e:
  errorMessage = "Exception occurred while executing script to generate input parameter for usp_get_phase_source_types " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
try:
  ...
  for i in range (0,2):
    try:
      ...
      spOutput = pd.read_sql_query("exec config.usp_get_phase_source_types {}".format(jBatchId[i]),conn)
    except Exception as e:
      executionOutputStatus = 'As expected'
      testCaseStatus = 'success'
      
      specificParameterTestScenarion = testCaseScenario + " for batch_id:'{}'".format(str(jBatchId[i]))
      #function to log the test case result in table
      ...
      logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 for stored procedure usp_get_phase_source_types') 
except Exception as e:
  errorMessage = "Exception occurred while testing on scenario 1 for stored procedure usp_get_phase_source_types': " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
try:
  ...
  spOutput = pd.read_sql_query("exec config.usp_get_phase_source_types {}".format(batchId),conn)
  
  #check the count of returned record by stored procedure
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       ...
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 2 stored procedure usp_get_source_types')  
except Exception as e:
  errorMessage="Exception occurred while testing scenario 2 stored procedure usp_get_sources_types : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  ...
  spOutputSourceType = spOutput[spOutput['source_type_name']=="DataSource"]
  
  #check the count of rows in dataframe if 1 then the returned result is correct
  if spOutputSourceType.shape[0] == 1:
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'    
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                    testCaseScenario,executionOutputStatus,sampleOutputLocation,
                    datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 3 stored procedure usp_get_phase_sources_types')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 3 stored procedure usp_get_phase_sources_types : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
