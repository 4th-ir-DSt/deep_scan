...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>Unit test for unit_testing_usp_get_mapping_object_attribute_names</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Unit test for unit_testing_usp_get_mapping_object_attribute_names stored procedure </td></tr>
...
try :
  ...
  testObjectName = 'uspGetMappingAttributeNames'
...
try:
  #run cleanup script
  cleanupMetadata = open("{}uspGetMappingAttributeNamesCleanUp.sql".format(testInputPath), "r").read()
...
try:
  #run cleanup script
  metadataPrep = open("{}uspGetMappingAttributeNamesDataPrep.sql".format(testInputPath), "r").read()
...
# DBTITLE 1,Scenario-1 Verify mapping columns between object attribute name and hash attribute type code
# Check if the column object attribute name 'TestGetMappingPiiFields_SurName' matches with the hash attribute type code'TestGetMappingPiiFields_SN'
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify mapping columns for source and destination objects' 
  #Metadata values
  sourceObjectName = 'TestGetMappingPiiFields_Source'
  sourceMappingField = 'TestGetMappingPiiFields_SurName'
  destinationMappingfield = 'TestGetMappingPiiFields_SN'
  # Execute the stored procedure
  spOutput = pd.read_sql_query("exec config.usp_get_mapping_object_attribute_names {}".format(sourceObjectName), conn)
  #Get the stor proc output source and target columns
  spOutputSourceField = spOutput.at[0,'object_attribute_name']
  spOutputDestinationField = spOutput.at[0,'hash_attribute_type_code']

  #If stored procedure output columns and metdata columns mathces then make test case as success
  if spOutputSourceField == sourceMappingField and spOutputDestinationField == destinationMappingfield :
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_get_mapping_pii_fields')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 1 stored procedure usp_get_mapping_pii_fields : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
