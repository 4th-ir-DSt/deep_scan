...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_log_data_quality_test_result</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>unit test stored procedure usp_log_data_quality_test_result</td></tr>
...
try :
  ...
  testObjectName = 'uspLogDataQualityTestResult'  
  requiredInputParameter = 'batchTaskId,dataQualityTestId,dataQualityTestResult,dataQualityTestMessage'
  testCaseScenario = ''
...
#run cleanup script
try:
  cleanup = open("{}UspLogDataQualityTestResultsCleanUp.sql".format(testInputPath), "r").read()
...
try:
  inputParameter = open('{}UspLogDataQualityTestResultDataPrep1.sql'.format(testInputPath), "r").read()
  ...
  newDataQualityTestId = int(inputParameterResults.at[0,'data_quality_test_id'])
  dataQualityTestResult = 'succeeded'
  dataQualityTestMessage = 'Test case succeess'
...
# DBTITLE 1,Scenario-1 Verify stored procedure logging status in tbl_data_quality_test_result
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify stored procedure logging status in tbl_data_quality_test_result'
  #Execute stored procedure
  cursor.execute("exec [audit].[usp_log_data_quality_test_result]?,?,?,?",newBatchTaskId, newDataQualityTestId, dataQualityTestResult,                                  dataQualityTestMessage)
  while cursor.nextset():
    x = 1
  #Get records for the inserted   
  spInsertedResult = pd.read_sql_query("SELECT * FROM audit.tbl_data_quality_test_result WHERE batch_task_id = '{}'".format(newBatchTaskId), conn)  
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_log_data_quality_test_result')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 1 stored procedure usp_log_data_quality_test_result : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  ...
  try:
    cursor.execute("exec [audit].[usp_log_data_quality_test_result]?,?,?,?",newBatchTaskId, newDataQualityTestId, dataQualityTestResult)
    while cursor.nextset():
      x = 1
  except Exception as e:
    executionOutputStatus = 'As expected'
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 2 stored procedure usp_log_data_quality_test_result')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 2 stored procedure usp_log_data_quality_test_result : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  #run cleanup script
  cleanupMetadata = open("{}UspLogDataQualityTestResultsCleanUp.sql".format(testInputPath), "r").read()
