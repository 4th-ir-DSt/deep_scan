...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_get_object_pii_hash_details_target_mapped</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>unit test stored procedure usp_get_object_pii_hash_details_target_mapped</td></tr>
...
try :
  ...
  testObjectName = 'uspGetObjectPiiHashDetailsTargetMapped'  
...
#run cleanup script
try:
  cleanup = open("{}GetObjectPiiHashDetailsTargetMappedCleanUp.sql".format(testInputPath), "r").read()
...
try:
  insertData = open('{}GetObjectPiiHashDetailsTargetMappedDataPrep.sql'.format(testInputPath), "r").read()
...
try:
  ...
  sourceObject = 'GetObjectPiiHashDetailsTargetMapped_a'
  sourceID = 10
  #While preparing data version 2 is the active version
  activeVersion = 2
  #Get object_id for testObject
  testSourceInputDf = spark.createDataFrame(pd.read_sql_query("select object_id from config.tbl_object where object_name = '{}'".format(sourceObject),conn))
  testSourceObjectId = testSourceInputDf.collect()[0][0]
  #Execute stored procedure
  spOutput = spark.createDataFrame(pd.read_sql_query("exec config.usp_get_object_pii_hash_details_target_mapped {},{}".format(testSourceObjectId,sourceID), conn))
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_get_object_pii_hash_details_target_mapped')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 1 stored procedure usp_get_object_pii_hash_details_target_mapped : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 2 stored procedure usp_get_object_pii_hash_details_target_mapped')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 2 stored procedure usp_get_object_pii_hash_details_target_mapped : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False

# COMMAND ----------

# DBTITLE 1,Scenario-3 Verify piiExpression to check stored proc returning destination attribute_type_code
#If multiple hash_attribute_type_code_id values for object then all attribute names will construct piiExpression value based on ascending order of hash_attribute_type_code_id and it will return destination attribute_type_code
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  ...
  #Expected output will come based on hash_attribute_type_code_id ascending order and it will return destination attribute_type_code
  expectedOutput = "col(str('TestSourceName')),col(str('TestSourceId'))"
  #Execute stored procedure
  spOutput = spark.createDataFrame(pd.read_sql_query("exec config.usp_get_object_pii_hash_details_target_mapped {},{}".format(testSourceObjectId,sourceID), conn))
  piiExpression = spOutput.collect()[0][1]
  ...
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 3 stored procedure usp_get_object_pii_hash_details_target_mapped')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 3 stored procedure usp_get_object_pii_hash_details_target_mapped : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False

# COMMAND ----------

# DBTITLE 1,Scenario-4 Verify piiTreaceabilityExpression contains stored proc parameters
...
try:
  ...
  sourceId = 1001
  tableName = 'GetObjectPiiHashDetailsTargetMapped_a'
  #Execute stored procedure
  spOutput = spark.createDataFrame(pd.read_sql_query("exec config.usp_get_object_pii_hash_details_target_mapped {},{},{}".format(testSourceObjectId,sourceId,tableName), conn))
  piiTreaceabilityExpression = spOutput.collect()[0][2]
  ...
  if str(sourceId) in piiTreaceabilityExpression and tableName in piiTreaceabilityExpression and 'StagingCreatedBatchID' in piiTreaceabilityExpression:
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'
  #function call to insert a row into tbl_unit_test on the test case status  
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 4 stored procedure usp_get_object_pii_hash_details_target_mapped')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 4 stored procedure usp_get_object_pii_hash_details_target_mapped : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
#Stored procedure need 3 parameters and 3 optional parameters. ObjectId and sourceId must need.If passing only 2 parameter it has to fail
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  ...
  try:
    spOutput = spark.createDataFrame(pd.read_sql_query("exec config.usp_get_object_pii_hash_details_target_mapped {}".format(testSourceObjectId), conn))
  except Exception as e:
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'
  #function call to insert a row into tbl_unit_test on the test case status  
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,sampleOutputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 5 stored procedure usp_get_object_pii_hash_details_target_mapped')    
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 5 stored procedure usp_get_object_pii_hash_details_target_mapped : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
...
try:
  #run cleanup script
  cleanupMetadata = open("{}GetObjectPiiHashDetailsTargetMappedCleanUp.sql".format(testInputPath), "r").read()
