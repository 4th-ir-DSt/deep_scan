...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>Unit test for unit_testing_usp_gdpr_get_sources</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Unit test for usp_gdpr_get_sources stored procedure </td></tr>
...
try :
  ...
  testObjectName = 'uspGdprGetSources'
  requiredInputParameter = 'referenceTableName'
...
try:
  #run cleanup script
  cleanupMetadata = open("{}uspGdprGetSourcesCleanUp.sql".format(testInputPath), "r").read()
...
try:
  ...
  inputParameter = open('{}uspGdprGetSourcesDataPrep.sql'.format(testInputPath), "r").read()
...
# DBTITLE 1,Scenario-1 Verify stored procedure returning correct sourceId
# While preparing data refereceObjectName is 'testUspGdprGetReferenceTables_ref'.If stor proc returns same sourceId if it matches with query output then test case is pass 
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify stored procedure returning correct sourceId' 
  #reference object while preparing metadata
  referenceObjectName = 'testUspGdprGetSources_ref'
  spOutputDf = pd.read_sql_query("exec config.usp_gdpr_get_sources '{}'".format(referenceObjectName), conn)
  spSourceName = spOutputDf.at[0,'source_id']
  querySourceNameDf = pd.read_sql_query("select * from config.tbl_gdpr_data_retention where reference_table_name = '{}'".format(referenceObjectName), conn)
  querySourceName = querySourceNameDf.at[0,'source_id']
  
  #If objectnames matches then make test case as success
  if spSourceName == querySourceName:
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_gdpr_get_sources')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 1 stored procedure usp_gdpr_get_sources : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False
