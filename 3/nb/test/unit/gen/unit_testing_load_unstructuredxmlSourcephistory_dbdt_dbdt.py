...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>Unit test for unit_testing_load_unstructuredxmlSourcehistory_dbdt_dbdt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Unit test for load_unstructuredxmlSourcehistory_dbdt_dbdt notebook </td></tr>
...
try :
  ...
  outputLocation = ''
  testObject = 'notebook'
  testObjectName = 'loadunstructuredxmlSourceHistoryDbdtDbdt'
...
# MAGIC %sql
# MAGIC drop table if exists testLoadHistoryDbdtDbdt_a;
# MAGIC drop table if exists Stg_testLoadHistoryDbdtDbdt_b;

# COMMAND ----------

# DBTITLE 1,Remove the directory for previous runs
dbutils.fs.rm(tableInputPath + '/testLoadHistoryDbdtDbdt_a',recurse=True)
dbutils.fs.rm(tableInputPath + '/Stg_testLoadHistoryDbdtDbdt_b',recurse=True)

# COMMAND ----------

# DBTITLE 1,Create source and destination delta table
# MAGIC %sql
# MAGIC create table testLoadHistoryDbdtDbdt_a(
# MAGIC row_id string,
# MAGIC id     string,
# MAGIC QuoteDateTime timestamp,
# MAGIC date string,
# MAGIC formattedMessage String)
# MAGIC USING DELTA
# MAGIC PARTITIONED BY (formattedMessage,date)
# MAGIC Location "/mnt/dataquality/unit_tests/loadunstructuredxmlSourceHistoryDbdtDbdt/input/testLoadHistoryDbdtDbdt_a";
# MAGIC 
# MAGIC -- insert data into table
# MAGIC insert into testLoadHistoryDbdtDbdt_a  values('aaa','a','2020-01-01 10:10:10',20200630,'true');
# MAGIC insert into testLoadHistoryDbdtDbdt_a  values('bbb','b','2020-01-02 10:10:10',20200630,'true');
# MAGIC insert into testLoadHistoryDbdtDbdt_a  values('ccc','c','2020-01-03 10:10:10',20200630,'true');
# MAGIC insert into testLoadHistoryDbdtDbdt_a  values('ddd','d','2020-01-04 10:10:10',20200629,'true');
# MAGIC 
# MAGIC create table Stg_testLoadHistoryDbdtDbdt_b(
# MAGIC id     string,
# MAGIC QuoteDateTime timestamp)
# MAGIC USING DELTA
# MAGIC Location "/mnt/dataquality/unit_tests/loadunstructuredxmlSourceHistoryDbdtDbdt/input/Stg_testLoadHistoryDbdtDbdt_b";
...
try:
  #run cleanup script
  cleanupMetadata = open("{}LoadunstructuredxmlSourceHistoryDbdtDbdtCleanUp.sql".format(testInputPath), "r").read()
...
try:
  ...
  inputParmeter = open("{}LoadunstructuredxmlSourceHistoryDbdtDbdtDataPrep1.sql".format(testInputPath), "r").read()
  ...
  #insertion of data into object,task_object_map, object_definition and object_dates_availability tables
  dataPreparation = open("{}LoadunstructuredxmlSourceHistoryDbdtDbdtDataPrep2.sql".format(testInputPath), "r").read()
...
# DBTITLE 1,Create Reference DataFrames 
try:
  #Get the Dataframe for reference tables
  unstructuredxmlSourceSchemeRefDF,unstructuredxmlSourceIntermediayDF = createReferenceDataFrames(cursor
                                                             ,batchTaskId
                                                             ,errorMessage
                                                             ,adfPipelineName
                                                             ,clusterId
                                                             ,notebookName
                                                             ,errorLogFileLocation)
  logTaskProgress(cursor,batchTaskId,"Reference table dataframes are created")
  
except Exception as e:
  errorMessage="Unable Get reference table dataframes: " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False   

# COMMAND ----------

# DBTITLE 1,Generate Reference Dictionaries and broadcast
try:
  #Get the dictionaries For Reference table
  unstructuredxmlSourceSchemeRefDic=generateDict(unstructuredxmlSourceSchemeRefDF
                               ,batchId
                               ,cursor
                               ,batchTaskId
                               ,adfPipelineName
  ...
  unstructuredxmlSourceIntermediaryDic=generateDict(unstructuredxmlSourceIntermediayDF
                                  ,batchId
                                  ,cursor
                                  ,batchTaskId
                                  ,adfPipelineName
                                  ...
                                  ,notebookName
                                  ,errorLogFileLocation)
  logTaskProgress(cursor,batchTaskId,"Reference table dictionaries are generated from dataframes")
except Exception as e:
  errorMessage="Unable Get the Dictionaries for the dataframes: " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False   

# COMMAND ----------

# DBTITLE 1,Broadcast Dictionary Variables
try:
  #Broadcast the dictionaries for better Performance
  unstructuredxmlSourceSchemeRefDicBcst=sc.broadcast(unstructuredxmlSourceSchemeRefDic)
  unstructuredxmlSourceIntermediaryDicBcst=sc.broadcast(unstructuredxmlSourceIntermediaryDic)
  logTaskProgress(cursor,batchTaskId,"Dictionary varibles are broadcasted")
except Exception as e:
  errorMessage="Unable Create braod cast variables for reference dictionaries: " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False   

# COMMAND ----------

# DBTITLE 1,Register Lookup And Hash Functions
try:
  getLookupunstructuredxmlSourceRegisterFunctions()
  computeHashValueUdfRegistration()
  logTaskProgress(cursor,batchTaskId,"Lookup and Hash udfs are registered")
except Exception as e:
  errorMessage="Unable register lookup and udfs: " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False   
...
# DBTITLE 1,Where condition for historical data
whereExpression= "formattedMessage=true and date =20200630"
...
# DBTITLE 1,Get QuoteDateTime window
#Get min and max quote date time to logged in the dates availablity table.
if targetObjectName!='Staging_unstructuredxmlSourceQuote_BadMessage':
  QuoteDateTime=sourceSelectWhereDF.selectExpr("max(QuoteDateTime)").collect()[0]
  maxQuoteDateTime=QuoteDateTime[0]
else:
  maxQuoteDateTime=currentTs
...
try:
  logTaskProgress(cursor,batchTaskId,"Started writing the destination dataframes to target table")
  ...
  writeToTargetTableWithAppend(sourceSelectWhereDF
                              ...
                              ,notebookName
                              ,errorLogFileLocation)
  logTaskProgress(cursor,batchTaskId,"Finished writing the destination dataframes to target table")
except Exception as e:
...
try:
  ...
  destinationTable = 'Stg_testLoadHistoryDbdtDbdt_b' 
...
#While preparing metadata gave where condition 'formattedMessage=true and date =20200630',So verify if row count is 3 then make test case is success
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  ...
  destinationTable = 'Stg_testLoadHistoryDbdtDbdt_b' 
  funcOutput = spark.sql("select distinct(id) from {}".format(destinationTable))
  if funcOutput.count() == 3 :
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'
  #function call to insert a row into tbl_unit_test on the test case status  
  logUnitTestStatus(testObject,testObjectName,requiredInputParameter,
                       testCaseScenario,executionOutputStatus,outputLocation,
                       datetime.now(),testCaseStatus)
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 2 function')  
except Exception as e:
...
try:
  #execute the script and get batch_task_id and task_id
  inputParmeter = open("{}LoadunstructuredxmlSourceHistoryDbdtDbdtDataPrep3.sql".format(testInputPath), "r").read()
...
try:
  conn, cursor = markDatesLoaded(targetObjectId, sourceObjectId, currentTs, maxQuoteDateTime,dbConn, conn,cursor,newBatchTaskId,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
...
try:
  ...
  spOutput = pd.read_sql_query("SELECT output_datetime_to FROM audit.vw_object_dates_availability WHERE batch_task_id = {} AND output_object_id = {} AND load_successful = 1".format(newBatchTaskId,targetObjectId),conn)
  outPutDateTimeTo = spOutput.at[0,'output_datetime_to']
  if spOutput.shape[0] == 1 and str(outPutDateTimeTo) == '2020-01-03 10:10:10':
    testResult = 'success'
    executionOutputStatus = 'As expected'
