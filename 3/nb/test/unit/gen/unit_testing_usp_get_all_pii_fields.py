...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>Unit test for unit_testing_usp_get_all_pii_fields</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Unit test for usp_get_all_pii_fields stored procedure </td></tr>
...
# MAGIC     <td>Changed to use hash_attribute_name column instead of object_attribute_name</td>
# MAGIC   </tr>
# MAGIC </table>
# MAGIC 
...
try :
  ...
  testObjectName = 'uspGetAllPiiFields'
...
try:
  #run cleanup script
  cleanupMetadata = open("{}uspGetAllPiiFieldsCleanUp.sql".format(testInputPath), "r").read()
...
try:
  #run cleanup script
  metadataPrep = open("{}uspGetAllPiiFieldsDataPrep.sql".format(testInputPath), "r").read()
...
# DBTITLE 1,Scenario-1 Verify stored procedure is returning all pii columns of person tables
# While preparing data for peron table pii column is 'TestGetAllPiiFields_PersonName'.So, in stored procedure output list this test column has to be there
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify stored procedure is returning all pii columns of person tables' 
  #pii column for person table while preparing data
  testPiiColumn = 'TestGetAllPiiFields_PersonName'
  # Execute the stored procedure
  spOutput = pd.read_sql_query("exec config.usp_get_all_pii_fields", conn)
  #Convert to spark Dataframe 
  spOutputDf = spark.createDataFrame(spOutput)
  #Get partition schemes
  piiFieldsDf = spOutputDf.select('hash_attribute_name').where('hash_attribute_name == "'+testPiiColumn+'"')
  piiFieldCount = piiFieldsDf.count()
  piiFieldName = piiFieldsDf.collect()[0][0]
  #If stored procedure output column count is one then make test case as success
  if piiFieldCount == 1 and piiFieldName == testPiiColumn:
  ...
  logTaskProgress(cursor,batchTaskId,'Unit test performed on scenario 1 stored procedure usp_get_all_pii_fields')  
except Exception as e:
  errorMessage = "Exception occurred while testing scenario 1 stored procedure usp_get_all_pii_fields : " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False

# COMMAND ----------

# DBTITLE 1,Scenario- 2 Verify stored procedure has to return person table pii fields only
# While preparing data for staging table pii column is 'TestGetAllPiiFields_StagingName'.So, in stored procedure output list this staging table pii column has to skip 
try:
  #Declare variables
  executionOutputStatus = 'Mismatched'
  testCaseStatus = 'failed'
  testCaseScenario = 'Verify stored procedure has to return Person table pii only' 
  #pii column for Staging table while preparing data
  testPiiColumn = 'TestGetAllPiiFields_StagingName'
  # Execute the stored procedure
  spOutput = pd.read_sql_query("exec config.usp_get_all_pii_fields", conn)
  #Convert to spark Dataframe 
  spOutputDf = spark.createDataFrame(spOutput)
  #Get all pii fields 
  piiFieldCount = spOutputDf.select('hash_attribute_name').where('hash_attribute_name == "'+testPiiColumn+'"').count()
 
  #If stored procedure output columns count is 0 then it is skipping staging table pii column. then make test case as success
  if piiFieldCount == 0:
    executionOutputStatus = 'As expected'
    testCaseStatus = 'success'
  #function call to insert a row into tbl_unit_test on the test case status  
