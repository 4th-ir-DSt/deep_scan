...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>unit_testing_usp_log_task_progress</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>unit test stored procedure usp_log_task_progress</td></tr>
...
# MAGIC     <td>Dev rework changes for camel case function names, date variables and removal of error line</td>
# MAGIC   </tr>
# MAGIC   <tr>
# MAGIC     <td>  </td>
# MAGIC     <td>  </td>
...
try :
  ...
  testObjectName = 'usp_log_task_progress'  
  requiredInputParameter = 'batchTaskId,batchTaskProgressMessage'
...
try:  
  #Parameter for usp_log_task_progress
  batchTaskProgressMessage = 'current batch process is scheduled{}'.format(date)
  logTaskProgress(cursor,batchTaskId,'Declaration of input variables for stored procedure')
except Exception as e:
  errorMessage = "Exception occured while 'Declaration of input variables for stored procedure': " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 

# COMMAND ----------

# DBTITLE 1,Testing scenario prep1 script execution
#inserting data into batch task table
try:
  prep1 = open("{}LogTaskProgressPrep1.sql".format(testInputPath), "r").read()
  ...
  errorMessage = "Exception occured while 'Executing of prep1 script to generate a new batch task id for testing': " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False  

# COMMAND ----------

# DBTITLE 1,Execute log task progress stored procedure
#execute stored procedure to insert a new entry in tbl_batch_task_progress
try:
  #executing stored procedure usp_log_task_progress
  cursor.execute("exec audit.usp_log_task_progress ?,?",newBatchTaskID,batchTaskProgressMessage)
  while cursor.nextset():
      x = 1
  logTaskProgress(cursor,batchTaskId,' Successfully executed the stored procedure')
except Exception as e:
  errorMessage="Exception occured while executing stored procedure: " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False

# COMMAND ----------

# DBTITLE 1,Scenario 1 - Check for insert row in tbl_batch_task_progress
...
try:
  ...
  spOutput=pd.read_sql_query("select * from audit.tbl_batch_task_progress where batch_task_id = {} and batch_task_progress_message = '{}'".format(newBatchTaskID,batchTaskProgressMessage),conn)
  ...
  logTaskProgress(cursor,batchTaskId,'Successfully executed scenario1 test case for stored procedure usp_log_task_progress')
except Exception as e:
  errorMessage="Exception occured while executing scenario1 test case for stored procedure usp_log_task_progress: " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False 
...
#cleaning the meta data from batch task and batch task progress tables
try:
  cleanup = open("{}LogTaskProgressCleanup.sql".format(testInputPath), "r").read()
  ...
  errorMessage = "Exception occured while Executing of cleanup script for usp_log_task_start: " + str(e)
  logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
  assert False    
