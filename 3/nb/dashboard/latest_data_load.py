...
dbutils.widgets.text("datetime_required","2023-04-02 14:00:00","Enter the datetime [yyyy-mm-dd hh24:mi:ss] after which analysis is required : ")
...
select main_data_tbl.phase_name,main_data_tbl.source_name,
...
main_data_tbl.process,main_data_tbl.status,main_data_tbl.current_batch_id,
last_load_tbl.last_run_batch_id,last_load_tbl.last_run_status,
main_data_tbl.current_run_start_time,main_data_tbl.current_run_end_time,
...
(select max(bt.batch_id) max_batch_id,bt.phase_id,bt.source_id from audit.tbl_batch_task bt,config.tbl_source s where 
s.source_name <> 'BordereauxMDS' and s.source_id = bt.source_id and bt.batch_task_start_datetime >= '{datetime_required}' 
...
and batch_task_start_datetime >= '{datetime_required}' ) main_data_tbl ,
(select phase_name,entity_name,process, avg(time_taken_in_mins) as historical_average_run_time_in_mins from (
select case 
    when lower(bt.activity_name) like '%dim%' then 'warehouse_dims'
    when lower(bt.activity_name) like '%bridge%' then 'warehouse_bridges'
    when lower(bt.activity_name) like '%fact%' then 'warehouse_facts'
...
from audit.tbl_batch_task bt, config.tbl_task t , config.tbl_entity e , config.tbl_phase p, config.tbl_source s,
(select distinct batch_id,bt.phase_id,bt.source_id from audit.tbl_batch_task bt,config.tbl_source s where 
s.source_name <> 'BordereauxMDS' and s.source_id = bt.source_id group by bt.phase_id ,bt.source_id,bt.batch_id ) temp
...
) temp1 group by phase_name,entity_name,process ) avg_time_tbl,
(select case 
    when lower(bt.activity_name) like '%dim%' then 'warehouse_dims'
    when lower(bt.activity_name) like '%bridge%' then 'warehouse_bridges'
    when lower(bt.activity_name) like '%fact%' then 'warehouse_facts'
...
from audit.tbl_batch_task bt, config.tbl_task t , config.tbl_entity e , config.tbl_phase p, config.tbl_source s,
(select temp1.batch_id as max_batch_id,temp1.phase_id,temp1.source_id from (
...
from 
(select distinct batch_id,bt.phase_id,bt.source_id from audit.tbl_batch_task bt,config.tbl_source s 
where s.source_name <> 'BordereauxMDS' and s.source_id = bt.source_id) temp) temp1 where rowNum=2) temp
where batch_id=temp.max_batch_id and bt.task_id=t.task_id and t.entity_id=e.entity_id and 
bt.phase_id=temp.phase_id and bt.source_id=temp.source_id
...
order by main_data_tbl.current_run_start_time"""
...
main_tbl.time_taken_in_mins
 from (select temp1.batch_id,temp1.phase_name--,temp1.source_name,
 ,temp1.status,
...
pipeline_execution_starttime,pipeline_execution_endtime,pipeline_execution_status
 from audit.tbl_log_adf_pipeline_execution where pipeline_execution_starttime >= '{datetime_required}'
 and pipeline_parameters like '%|%') adf,
(select max(bt.batch_id) max_bid,bt.phase_id,bt.source_id from audit.tbl_batch_task bt,config.tbl_source s where 
s.source_name <> 'BordereauxMDS' and s.source_id = bt.source_id and bt.batch_task_start_datetime >= '{datetime_required}' 
...
(select top 1 'raw' as phase_name, 'pending' as status from config.tbl_source
UNION
select top 1 'standardised' as phase_name, 'pending' as status from config.tbl_source
UNION
select top 1 'conformed' as phase_name, 'pending' as status from config.tbl_source
UNION
select top 1 'staging-warehouse' as phase_name, 'pending' as status from config.tbl_source
UNION
select top 1 'warehouse_dims' as phase_name, 'pending' as status from config.tbl_source
UNION
select top 1 'warehouse_bridges' as phase_name, 'pending' as status from config.tbl_source
UNION
select top 1 'warehouse_facts' as phase_name, 'pending' as status from config.tbl_source
) dummy_tbl
on 
main_tbl.phase_name = dummy_tbl.phase_name) temp2
on b.batch_id=temp2.batch_id
order by COALESCE(temp2.batch_id, 999999999),2"""
...
s.source_id = bt.source_id and bt.batch_task_start_datetime >= '{datetime_required}') temp
...
and bt.batch_task_id=el.batch_task_id and el.error_datetime >= '{datetime_required}'"""
...
# DBTITLE 1,Tables Latest Load Status across Layer - TBD - Need to be removed
batchTaskQuery_entity = """select p.phase_name,bt.batch_task_status,count(*) as Table_Count from audit.tbl_batch_task bt , 
(select max(batch_id) max_bid,phase_id from audit.tbl_batch_task group by phase_id ) temp , config.tbl_phase p
where bt.batch_id = temp.max_bid and bt.phase_id = temp.phase_id and p.phase_id=bt.phase_id and p.phase_name <> 'warehouse'
group by p.phase_name,bt.batch_task_status
union
select 'warehouse_dims' as phase_name,bt.batch_task_status,count(*) as Table_Count from audit.tbl_batch_task bt ,
(select max(batch_id) max_bid,phase_id from audit.tbl_batch_task group by phase_id ) temp, config.tbl_phase p
where lower(bt.activity_name) like '%dim%' and temp.max_bid=bt.batch_id and bt.phase_id = temp.phase_id and p.phase_id=bt.phase_id
group by p.phase_name,bt.batch_task_status
union
select 'warehouse_bridges' as phase_name,bt.batch_task_status,count(*) as Table_Count from audit.tbl_batch_task bt ,
(select max(batch_id) max_bid,phase_id from audit.tbl_batch_task group by phase_id ) temp,config.tbl_phase p
where lower(bt.activity_name) like '%bridge%' and temp.max_bid=bt.batch_id and bt.phase_id = temp.phase_id and p.phase_id=bt.phase_id
group by p.phase_name,bt.batch_task_status
union
select 'warehouse_facts' as phase_name,bt.batch_task_status,count(*) as Table_Count from audit.tbl_batch_task bt ,
(select max(batch_id) max_bid,phase_id from audit.tbl_batch_task group by phase_id ) temp,config.tbl_phase p
where lower(bt.activity_name) like '%fact%' and temp.max_bid=bt.batch_id and bt.phase_id = temp.phase_id and p.phase_id=bt.phase_id
group by p.phase_name,bt.batch_task_status;"""


# pd_df = pd.read_sql_query(batchTaskQuery_entity,connMeta)
# # set the max columns to none
# pd.set_option('display.max_columns', None)
...
# DBTITLE 1,Last Load Total Run Time - TBD - Need to be removed
batchTaskQuery_total_time = """select b.Load_Start_Time,b.Load_End_Time,DATEDIFF(minute,b.Load_Start_Time,b.Load_End_Time) as Time_Taken_in_Mins from (
select min(a.pipeline_execution_starttime) as Load_Start_Time, max(a.pipeline_execution_endtime) as Load_End_Time from (
select batch_id,pipeline_execution_starttime,pipeline_execution_endtime,
ROW_NUMBER() OVER(Partition BY substring(pipeline_parameters,CHARINDEX('|',pipeline_parameters,2)+1,len(pipeline_parameters)) ORDER BY batch_id DESC ) Rn
FROM audit.tbl_log_adf_pipeline_execution where lower(pipeline_execution_status) = 'completed'
and pipeline_parameters like '%|%' 
) a where a.rn=1 )b"""

# pd_df = pd.read_sql_query(batchTaskQuery_total_time,connMeta)
# # set the max columns to none
# pd.set_option('display.max_columns', None)
