...
# MAGIC # Introduction
# MAGIC <table>
# MAGIC   <tr><td><b>Notebook Name</b></td><td>cur_unstructuredSource_dbdt_dbdbt</td></tr>
# MAGIC   <tr><td><b>One Liner</b></td><td>Loading data from raw delta to staging currnet state tables</td></tr>
...
try:
  ...
  from pyspark.sql.functions import lit,col,explode,concat_ws,create_map,struct,collect_list,coalesce,max
...
try:
  ...
  targetObjectExtendedProperties=srcDesDict['destination_extended_properties']
...
try:
  sourceSelectWhereDF = selectDestFromSrcTable(
                                          sourceTableName
...
# DBTITLE 1,Rebuild Current state table
#Get firstrow of the data frame to see whether rows present in source
sourceTabeExists=bool(sourceSelectWhereDF.head(1))
if sourceTabeExists:
  try:
      #Rebuild current state table  
      currentVersionDf= unstructuredSourceFunctions.reBuildCurrentStateTable(sourceSelectWhereDF
                                                                  ,targetObjectName
                                                                  ,columnListFinal
                                                                  ,cursor
                                                                  ,batchTaskId
                                                                  ...
                                                                  ,notebookName
                                                                  ,errorLogFileLocation)
      #Overwrite the currentstate table
      writeToTargetTableWithOverwrite(currentVersionDf
                                      ,targetObjectName
                                      ,targetFormat
                                      ,cursor,batchTaskId
                                      ,adfPipelineName
      ...
      errorMessage="Unable rebuild the current state tables: " + str(e)
      logError(cursor,batchTaskId,errorMessage,adfPipelineName,clusterId,notebookName,errorLogFileLocation)
      assert False  
