...
if TableName != "":
    raw_tbl_list = ['raw_fx_rate.'+TableName]
    print("::>>>>Raw TableName provided by User")
else:
    print("::>>>>All Table included from raw_fx_rate as no input was provided by user ")
    raw_tbl_list = spark.sql("SHOW TABLES IN raw_fx_rate").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
raw_test_result_string = ' '.join(raw_test_result).replace("raw_fx_rate.","")

# Executing SQL and storing result in Pandas DF 
spark.sql("use raw_fx_rate")
...
# # get primary keys from standardized MDM for Raw subscribe
# sqlObject = """select replace(lower(a.object_name),'Standardised_fx_rate.','') as tableName, b.object_attribute_name, 
...
# where lower(object_name) like 'standardised_fx_rate.%'
...
# else:
#     print("::>>>>All Table included from raw_fx_rate as no input was provided by user ")
#     raw_tbl_list = spark.sql("SHOW TABLES IN raw_fx_rate").where("database!=''").select("tableName").rdd.flatMap(lambda x: x).collect()
...
#             raw_test_result0 = "select '"+str(raw_tbl_list[raw_tbl])+"' as Table, count(1) as raw_cnt from raw_fx_rate."+str(raw_tbl_list[raw_tbl])+" group by '"+str(raw_tbl_list[raw_tbl])+"', concat_ws('_',"+str(raw_col_list[raw_col])+") having count(1) > 1 UNION ALL"
#             raw_test_result.append(raw_test_result0)
#     else:
#         for raw_col in range(len(raw_col_list)):
...
#                 raw_test_result.append(raw_test_result0)
#             else:
#                 raw_test_result0 = "select '"+str(raw_tbl_list[raw_tbl])+"' as Table, count(1) as raw_cnt from raw_fx_rate."+str(raw_tbl_list[raw_tbl])+" group by '"+str(raw_tbl_list[raw_tbl])+"', concat_ws('_',"+str(raw_col_list[raw_col])+") having count(1) > 1"
#                 raw_test_result.append(raw_test_result0)
...
display(spark.sql("""select 'exchange_rate_daily' as Table, Quote_Date, Base_Currency, Exchange_Rate_Source, count(1) from raw_fx_rate.exchange_rate_daily
group by Quote_Date, Base_Currency, Exchange_Rate_Source having count(1) > 1"""))
...
if TableName != "":
    stand_tbl_list = ['standardised_fx_rate.'+TableName]
    print("::>>>>Standardised TableName provided by User")
else:
    print("::>>>>All Table included from standardised_fx_rate as no input was provided by user ")
    stand_tbl_list = spark.sql("SHOW TABLES IN standardised_fx_rate").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
# Converting list to string
stand_test_result_string = ' '.join(stand_test_result).replace("standardised_fx_rate.","")

# Executing SQL and storing result in Pandas DF 
spark.sql("use standardised_fx_rate")
...
# DBTITLE 1,Record Count Comparison Raw v/s Standardised Tables
...
stand_in_tbl = ''
stand_tbl_list = spark.sql("SHOW TABLES IN standardised_fx_rate").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").collect()
...
stand_run_in_tbl = ''
in_stand_ver_num = ''
stand_tbl_list = spark.sql("SHOW TABLES IN standardised_fx_rate").where("database!=''").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").collect()
for row in stand_tbl_list:
...
if TableName != "":
    conf_tbl_list = ['conformed_fx_rate.'+TableName]
    print("::>>>>Conformed TableName provided by User")
else:
    print("::>>>>All Table included from conformed_fx_rate as no input was provided by user ")
    conf_tbl_list = spark.sql("SHOW TABLES IN conformed_fx_rate").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
conf_in_tbl = ''
conf_tbl_list = spark.sql("SHOW TABLES IN conformed_fx_rate").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").collect()
for row in conf_tbl_list:
    ...
    x=spark.sql("""select hashedbusinesskey, lakeValidFromTimestamp, count(1) 
                   from {} group by hashedbusinesskey, lakeValidFromTimestamp having count(1)>1 
                   UNION ALL 
                   select hashedbusinesskey, NULL AS lakeValidFromTimestamp, count(1) from {} 
                   where LakeIsActive = 1 group by hashedbusinesskey having count(1)>1""".format(conf_in_tbl,conf_in_tbl))
    if x.count()>1:
...
# DBTITLE 1,Identify the column containing NULL & Blank values - Can have only metrics
test_result = []
...
if TableName != "":
    tbl_list = ['conformed_fx_rate.'+TableName]
    print("::>>>>Conformed TableName provided by User")
else:
    print("::>>>>All Table included from conformed_fx_rate as no input was provided by user ")
    tbl_list = spark.sql("SHOW TABLES IN conformed_fx_rate").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
if TableName != "":
    conf_tbl_list_1 = ['conformed_fx_rate.'+TableName]
    print("::>>>>Conformed TableName provided by User")
else:
    print("::>>>>All Table included from conformed_fx_rate as no input was provided by user ")
    conf_tbl_list_1 = spark.sql("SHOW TABLES IN conformed_fx_rate").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
