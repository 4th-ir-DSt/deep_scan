...
if TableName != "":
    raw_tbl_list = ['raw_subscribe.'+TableName]
    print("::>>>>Raw TableName provided by User")
else:
    print("::>>>>All Table included from raw_subscribe as no input was provided by user ")
    raw_tbl_list = spark.sql("SHOW TABLES IN raw_subscribe").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
raw_test_result_string = ' '.join(raw_test_result).replace("raw_subscribe.","")

# Executing SQL and storing result in Pandas DF 
spark.sql("use raw_subscribe")
...
sqlObject = """select replace(lower(a.object_name),'Standardised_subscribe.','') as tableName, b.object_attribute_name, 
...
where lower(object_name) like 'standardised_subscribe.dbo_%' and lower(object_name) not like '%intrali%' 
...
if TableName != "":
    ...
    print("::>>>>All Table included from raw_subscribe as no input was provided by user ")
    raw_tbl_list = spark.sql("SHOW TABLES IN raw_subscribe").where("database!=''").select("tableName").rdd.flatMap(lambda x: x).collect()
...
#     print(raw_col_list)
    if raw_tbl !=len(raw_tbl_list)-1:
        for raw_col in range(len(raw_col_list)):
            raw_test_result0 = "select '"+str(raw_tbl_list[raw_tbl])+"' as Table, count(1) as raw_cnt from raw_subscribe."+str(raw_tbl_list[raw_tbl])+" group by '"+str(raw_tbl_list[raw_tbl])+"', concat_ws('_',"+str(raw_col_list[raw_col])+") having count(1) > 1 UNION ALL"
            raw_test_result.append(raw_test_result0)
    else:
        for raw_col in range(len(raw_col_list)):
            if raw_col != len(raw_col_list)-1:
                ...
                raw_test_result0 = "select '"+str(raw_tbl_list[raw_tbl])+"' as Table, count(1) as raw_cnt from raw_subscribe."+str(raw_tbl_list[raw_tbl])+" group by '"+str(raw_tbl_list[raw_tbl])+"', concat_ws('_',"+str(raw_col_list[raw_col])+") having count(1) > 1"
                raw_test_result.append(raw_test_result0)
...
if TableName != "":
    stand_tbl_list = ['standardised_subscribe.'+TableName]
    print("::>>>>Standardised TableName provided by User")
else:
    print("::>>>>All Table included from standardised_subscribe as no input was provided by user ")
    stand_tbl_list = spark.sql("SHOW TABLES IN standardised_subscribe").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
# Converting list to string
stand_test_result_string = ' '.join(stand_test_result).replace("standardised_subscribe.","")

# Executing SQL and storing result in Pandas DF 
spark.sql("use standardised_subscribe")
...
stand_in_tbl = ''
stand_tbl_list = spark.sql("SHOW TABLES IN standardised_subscribe").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").collect()
...
stand_run_in_tbl = ''
in_stand_ver_num = ''
stand_tbl_list = spark.sql("SHOW TABLES IN standardised_subscribe").where("database!=''").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").collect()
for row in stand_tbl_list:
...
if TableName != "":
    conf_tbl_list = ['conformed_subscribe.'+TableName]
    ...
    conf_tbl_list = spark.sql("SHOW TABLES IN conformed_subscribe").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
print("       1.) claim_movement")
print("       2.) claim_relationship")
print("       3.) document")
print("       4.) item_coverage_characteristic")
print("       5.) party_claim_activity_role")
print("       6.) party_claim_movement_role")
print("       7.) party_claim_transaction_role")
print("       8.) party_deduction_role")
print("       9.) party_rating")
print("       10.) party_relationship")
print("       11.) person")
print("       12.) party_rating")

# COMMAND ----------

# DBTITLE 1,Grain Check for conformed tables - should get no records in output
conf_in_tbl = ''
conf_tbl_list = spark.sql("SHOW TABLES IN conformed_subscribe").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").collect()
...
if TableName != "":
    tbl_list = ['conformed_subscribe.'+TableName]
    print("::>>>>Conformed TableName provided by User")
else:
    print("::>>>>All Table included from conformed_subscribe as no input was provided by user ")
    tbl_list = spark.sql("SHOW TABLES IN conformed_subscribe").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
if TableName != "":
    conf_tbl_list_1 = ['conformed_subscribe.'+TableName]
    print("::>>>>Conformed TableName provided by User")
else:
    print("::>>>>All Table included from conformed_subscribe as no input was provided by user ")
    conf_tbl_list_1 = spark.sql("SHOW TABLES IN conformed_subscribe").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
