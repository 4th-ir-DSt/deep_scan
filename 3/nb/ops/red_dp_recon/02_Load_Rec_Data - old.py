 ...
 SELECT
      ...
      , ((ROUND(COALESCE(USM_Additional_Premium_Gross,0),2)) 
        ...
        --    + (ROUND(COALESCE(USM_Profit_Commission_Return_Premium_Gross,0),2))
            + (ROUND(COALESCE(USM_Return_Premium_Gross,0),2))
            ) AS Line_Gross_Gross_Signed_Premium_SCC_RED
...
(
  ...
  WHERE dpclaim.measure_name in ('Line_Gross_Outstanding_Claim_SCC', 'Line_Gross_Paid_Claim_SCC', 'Line_Gross_Outstanding_Claim_Fee_SCC', 'Line_Gross_Paid_Claim_Fee_SCC'
                                      , 'Line_Gross_Reported_Outstanding_Claim_SCC')
...
PIVOT 
(
SUM(CLAIMS_AMOUNT) FOR measure_name IN ('Line_Gross_Paid_Claim_SCC','Line_Gross_Reported_Outstanding_Claim_SCC', 'Line_Gross_Outstanding_Claim_Fee_SCC', 'Line_Gross_Paid_Claim_Fee_SCC')
) 
ORDER BY policy_reference
''')#.display()
...
(
  ...
  WHERE redclaim.measure_name in ('Gross_Claim_SCM_OS', 'Gross_Claim_SCM_Paid', 'Gross_Claim_SCM_OS_Fees', 'Gross_Claim_SCM_Paid_Fees')
...
PIVOT 
(
SUM(CLAIMS_AMOUNT) FOR measure_name IN ('Gross_Claim_SCM_OS', 'Gross_Claim_SCM_Paid', 'Gross_Claim_SCM_OS_Fees', 'Gross_Claim_SCM_Paid_Fees')
) 
ORDER BY policy_reference
''')#.display()
...
select
  ...
  , case
      when Measure in (
                        'Line_Gross_Paid_Claim_SCC_DP', 'Line_Gross_Outstanding_Claim_SCC_DP', 'Line_Gross_Outstanding_Claim_Fee_SCC_DP', 'Line_Gross_Paid_Claim_Fee_SCC_DP'
                      )
      then 'DP'
      when Measure in (
                        'Line_Gross_Paid_Claim_SCC_RED', 'Line_Gross_Outstanding_Claim_SCC_RED', 'Line_Gross_Outstanding_Claim_Fee_SCC_RED', 'Line_Gross_Paid_Claim_Fee_SCC_RED'
                      )
      then 'RED'
      when Measure in (
                        'Line_Gross_Paid_Claim_SCC_VAR', 'Line_Gross_Outstanding_Claim_SCC_VAR', 'Line_Gross_Outstanding_Claim_Fee_SCC_VAR', 'Line_Gross_Paid_Claim_Fee_SCC_VAR'
                      )
      then 'VAR'
    end as Data_Type
   , Measure
