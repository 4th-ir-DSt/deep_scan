...
if TableName != "":
    raw_tbl_list = ['raw_mdm.'+TableName]
    print("::>>>>Raw TableName provided by User")
else:
    print("::>>>>All Table included from raw_mdm as no input was provided by user ")
    raw_tbl_list = spark.sql("SHOW TABLES IN raw_mdm").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
raw_test_result_string = ' '.join(raw_test_result).replace("raw_mdm.","")

# Executing SQL and storing result in Pandas DF 
spark.sql("use raw_mdm")
...
# get primary keys from standardized MDM for Raw MDM
sqlObject = """select replace(lower(a.object_name),'Standardised_MDM.','') as tableName, b.object_attribute_name, 
...
where lower(object_name) like 'standardised_mdm.mdm_%' and lower(object_name) not like '%intrali%' 
...
raw_tbl_col = ( dfget_Keys.filter(~dfget_Keys.object_attribute_name.like("%Year%")).groupby("tableName")
                           .agg(concat_ws(", ", collect_list(dfget_Keys.object_attribute_name)).alias("primary_keys")) )
...
if TableName != "":
    ...
    print("::>>>>All Table included from raw_mdm as no input was provided by user ")
    raw_tbl_list = spark.sql("SHOW TABLES IN raw_mdm").where("database!=''").select("tableName").rdd.flatMap(lambda x: x).collect()
...
#     print(raw_col_list)
    if raw_tbl !=len(raw_tbl_list)-1:
        for raw_col in range(len(raw_col_list)):
            raw_test_result0 = "select '"+str(raw_tbl_list[raw_tbl])+"' as Table, count(1) as raw_cnt from raw_mdm."+str(raw_tbl_list[raw_tbl])+" group by '"+str(raw_tbl_list[raw_tbl])+"', concat_ws('_',"+str(raw_col_list[raw_col])+") having count(1) > 1 UNION ALL"
            raw_test_result.append(raw_test_result0)
    else:
        for raw_col in range(len(raw_col_list)):
            if raw_col != len(raw_col_list)-1:
                ...
                raw_test_result0 = "select '"+str(raw_tbl_list[raw_tbl])+"' as Table, count(1) as raw_cnt from raw_mdm."+str(raw_tbl_list[raw_tbl])+" group by '"+str(raw_tbl_list[raw_tbl])+"', concat_ws('_',"+str(raw_col_list[raw_col])+") having count(1) > 1"
                raw_test_result.append(raw_test_result0)
                
                
raw_test_result_string = ' '.join(raw_test_result)

# print(raw_test_result_string)
...
# Exception
print("Check failure needs to be excluded for following entities:")
print("       1.) mdm_loss_event_large_loss_threshold")
print("       2.) mdm_related_leader_mapping")
print("       3.) mdm_planning_classification")
print("       4.) mdm_business_entity_group")
...
if TableName != "":
    stand_tbl_list = ['standardised_mdm.'+TableName]
    print("::>>>>Standardised TableName provided by User")
else:
    print("::>>>>All Table included from standaedised_mdm as no input was provided by user ")
    stand_tbl_list = spark.sql("SHOW TABLES IN standardised_mdm").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
    stand_tbl_list.remove('standardised_mdm.vw_latest_intrali_aggregationmatrixconfigurationtable')
    stand_tbl_list.remove('standardised_mdm.vw_latest_mdm_intrali_masterreferencetable')
...
# Converting list to string
stand_test_result_string = ' '.join(stand_test_result).replace("standardised_mdm.","")

# Executing SQL and storing result in Pandas DF 
spark.sql("use standardised_mdm")
...
# Exception
print("Check failure needs to be excluded for following entities:")
print("       1.) mdm_business_classification_mapping")
print("       2.) mdm_business_entity_group")
print("       3.) mdm_claim_business_class_hierarchy_mapping")
print("       4.) mdm_planning_classification")
print("       5.) mdm_reserving_class_hierarchy_mapping")

# COMMAND ----------

# DBTITLE 1,Grain Check for standardised tables - should get no records in output
stand_in_tbl = ''
stand_tbl_list = spark.sql("SHOW TABLES IN standardised_mdm").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").rdd.flatMap(lambda x:x).collect()
# stand_tbl_list= stand_tbl_list['table_name']
stand_tbl_list.remove('standardised_mdm.vw_latest_intrali_aggregationmatrixconfigurationtable')
stand_tbl_list.remove('standardised_mdm.vw_latest_mdm_intrali_masterreferencetable')
# print(stand_tbl_list)
...
stand_run_in_tbl = ''
in_stand_ver_num = ''
stand_tbl_list = spark.sql("SHOW TABLES IN standardised_mdm").where("database!=''").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").where("tableName not like 'vw%'").collect()
for row in stand_tbl_list:
...
if TableName != "":
    conf_tbl_list = ['conformed_mdm.'+TableName]
    print("::>>>>Conformed TableName provided by User")
else:
    print("::>>>>All Table included from conformed_mdm as no input was provided by user ")
    conf_tbl_list = spark.sql("SHOW TABLES IN conformed_mdm").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
for conf_tbl in range(len(conf_tbl_list)):
    if conf_tbl !=len(conf_tbl_list)-1:
        conf_test_result0 = "select '"+str(conf_tbl_list[conf_tbl])+"' as Table, count(1) as cnf_cnt from "+str(conf_tbl_list[conf_tbl])+" WHERE lakeIsActive = True and lakeDeletedTimestamp IS NULL UNION ALL"
        conf_test_result.append(conf_test_result0)
    else:
        conf_test_result0 = "select '"+str(conf_tbl_list[conf_tbl])+"' as Table, count(1) as cnf_cnt from "+str(conf_tbl_list[conf_tbl])+" WHERE lakeIsActive = True and lakeDeletedTimestamp IS NULL"
        conf_test_result.append(conf_test_result0)
...
# Showing columns having null values
# display(conf_finaldf[conf_finaldf['count(1)'] > 0])
display(conf_finaldf.filter(conf_finaldf["cnf_cnt"] == 0))

# COMMAND ----------

# DBTITLE 1,Record Count Comparison of Standardised vs Conformed Tables - should get no records in output
conf_finaldf_1 = conf_finaldf.withColumn("Table",regexp_replace(conf_finaldf.Table,"conformed_mdm.",""))
conf_finaldf_2 = conf_finaldf_1.withColumn("Table",concat(lit("mdm_"),conf_finaldf_1.Table))

stand_conf_cnt = stand_finaldf.join(conf_finaldf_2, stand_finaldf["Table"] == conf_finaldf_2["Table"], "outer")
display(stand_conf_cnt.filter(stand_conf_cnt["stand_cnt"] != stand_conf_cnt["cnf_cnt"]))
...
conf_in_tbl = ''
conf_tbl_list = spark.sql("SHOW TABLES IN conformed_mdm").where("database!=''").select(concat("database",lit("."),"tableName")).withColumnRenamed("concat(database, ., tableName)", "table_name").collect()
...
tbl_list = ['conformed_mdm.'+TableName]
test_result = []
...
if TableName != "":
    ...
    print("::>>>>All Table included from conformed_mdm as no input was provided by user ")
    tbl_list = spark.sql("SHOW TABLES IN conformed_mdm").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
...
if TableName != "":
    conf_tbl_list_1 = ['conformed_mdm.'+TableName]
    print("::>>>>Conformed TableName provided by User")
else:
    print("::>>>>All Table included from conformed_subscribe as no input was provided by user ")
    conf_tbl_list_1 = spark.sql("SHOW TABLES IN conformed_mdm").where("database!=''").select(concat("database",lit("."),"tableName")).rdd.flatMap(lambda x: x).collect()
