...
dbutils.widgets.dropdown("sfIDLTable", "SFMC_SENT", 
                         ["SFMC_SENT"], "Snowflake table ")
...
cdm_table      = 'trg_mktg_email_sent_plr'
stage_cdm_table      = 'trg_mktg_email_sent_plr'
...
df = df.withColumn("SfmcSentID", concat(col("SubscriberKey"),lit("-"), col("BatchID"),lit("-"), col("JobID")))
...
if nrows > 0:  
  ...
  try:
    df.write \
      ...
      .mode(mode) \
      .save()
    print("{}.{} successfully loaded into Snowflake stage table {} !".format(schema, sfTable, stage_cdm_table))
  except:
...
if nrows > 0: 
  if IDLtype == 'Upsert':
    ...
    ON SNOW.SfmcSentID = CDM.SfmcSentID

    WHEN MATCHED AND 
    (
        not(equal_null(SNOW.SfmcSentID,CDM.SfmcSentID))
    ...
    THEN UPDATE SET 
        SNOW.SfmcSentID                       = CDM.SfmcSentID
