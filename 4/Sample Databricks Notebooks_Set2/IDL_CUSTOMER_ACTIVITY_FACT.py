...
dbutils.widgets.dropdown("sfIDLTable", "CUSTOMER_ACTIVITY_FACT", 
                         ["CUSTOMER_ACTIVITY_FACT"], "Snowflake table ")
...
nbName   = "Prod_IDL_CUSTOMER_ACTIVITY_FACT"
tz       = timezone('EST')
...
cdm_table      = 'trg_mktg_activity_plr'
stage_cdm_table      = 'trg_mktg_activity_plr'
...
# DBTITLE 1,Add primary key
# Add primary key column: (dnkn) PointActivityId, BrandId, Source, CertificateNumber
#                         (other brands) PointActivityId, BrandId, Source
df = df.withColumn('CustomerActivityId', when(df.BrandId == 'dnkn',
                                         concat_ws("", col("PointActivityId"), col("BrandId"),col("Source"), col("CertificateNumber")))
                                         .otherwise(concat_ws("", col("PointActivityId"), col("BrandId"), col("Source"))))
...
if nrows > 0:  
  if truncate == 'True':
    ...
    try:
      ...
      print("Error truncating table {}.{} in Snowflake !".format(schemaSTAGE, stage_cdm_table))
  else:
    mode = "append"
  ...
  try:
    df.write \
      ...
      .mode(mode) \
      .save()
    print("{}.{} successfully loaded into Snowflake stage !".format(schema, cdm_table))
  except:
...
if nrows > 0: 
  if IDLtype == 'Upsert':
    ...
    SELECT 
        ...
        ,TO_TIMESTAMP(CreatedDate) CreatedDate
        ,CdmLoadDate
    ...
    ON SNOW.CustomerActivityId = CDM.CustomerActivityId

    WHEN MATCHED AND 
    (
      not(equal_null(SNOW.CustomerActivityId,CDM.CustomerActivityId))  
      OR not(equal_null(SNOW.ProfileId,CDM.MemberId))  
      OR not(equal_null(SNOW.PointActivityId,CDM.PointActivityId))  
      ...
      OR not(equal_null(SNOW.Points,CDM.Points))  
      OR not(equal_null(SNOW.ActivityTypeDescription,CDM.ActivityTypeDescription))  
      OR not(equal_null(SNOW.StoreLocation,CDM.StoreLocation))  
      ...
      OR not(equal_null(SNOW.ActivityTypeGroup,CDM.ActivityTypeGroup))  
      OR not(equal_null(SNOW.ActivityType,CDM.ActivityType))  
      OR not(equal_null(SNOW.Privacy,CDM.Privacy))  
      OR not(equal_null(SNOW.CreatedDate,CDM.CreatedDate))  
      OR not(equal_null(SNOW.CdmLoadDate,CDM.CdmLoadDate))  
    )

    THEN UPDATE SET 
      SNOW.CustomerActivityId       =  CDM.CustomerActivityId  
      ,SNOW.ProfileId               =  CDM.MemberId  
      ,SNOW.PointActivityId         =  CDM.PointActivityId  
      ,SNOW.TransactionId           =  CDM.TransactionId  
      ,SNOW.BrandId                 =  CDM.BrandId  
      ,SNOW.MemberCardNumber        =  CDM.MemberCardNumber  
      ,SNOW.Points                  =  CDM.Points  
      ,SNOW.ActivityTypeDescription =  CDM.ActivityTypeDescription  
      ...
      ,SNOW.ActivityTypeGroup       =  CDM.ActivityTypeGroup  
      ,SNOW.ActivityType            =  CDM.ActivityType  
      ,SNOW.Privacy                 =  CDM.Privacy  
      ,SNOW.CreatedDate             =  CDM.CreatedDate  
    ...
    VALUES   (
      ...
      ,CDM.Points
      ,CDM.ActivityTypeDescription
      ,CDM.StoreLocation
      ,CDM.TransactionNumber
      ,CDM.CertificateNumber
      ,CDM.ActivityTypeGroup
      ,CDM.ActivityType
      ,CDM.Privacy
      ,CDM.CreatedDate
      ,CDM.CdmLoadDate
...
cdmCount = sqlContext.sql(query.format(pk_st,pk_st, databricks, cdm_table))
