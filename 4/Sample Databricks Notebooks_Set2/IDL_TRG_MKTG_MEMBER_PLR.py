...
dbutils.widgets.dropdown("sfIDLTable", "trg_mktg_member_plr", 
                         ["trg_mktg_member_plr"], "Snowflake table ")
...
cdmDBEnv = dbutils.widgets.get("DataBricks")

# Execution ID = Name of Notebook + Timestamp with Millisecond + Random Number
nbName   = "Prod_IDL_LOYALTY_MEMBERSHIP_PLR"
tz       = timezone('EST')
...
print('SELECT * FROM ' + cdmDBEnv + '.' + cdm_table)

if delta == '*':
  loadDate = 0
  df = sqlContext.sql("SELECT * FROM {}.{} ".format(cdmDBEnv,cdm_table))
  
elif delta == 'sfMax':
  # Get current max date in Snowflake table
  loadDate = curSTAGE.execute("SELECT MAX(CAST(CdmLoadDate AS INT)) FROM {}.{}".format(schemaSTAGE, sfTable)).fetchone()[0]
  if loadDate is None:
    loadDate = 0
  df = sqlContext.sql("SELECT * FROM {}.{} WHERE CAST(CdmLoadDate AS INT) > '{}'".format(cdmDBEnv,cdm_table, loadDate))
  print('Snowflake max date in {}.{} is: '.format(schemaSTAGE, sfTable) + str(loadDate))
...
if nrows > 0:  
  if truncate == 'True':
    mode = "overwrite"
    try:
      curSTAGE.execute("TRUNCATE TABLE {}.{}".format(schemaSTAGE, sfTable))      
    except: 
      print("Error truncating table {}.{} in Snowflake !".format(schemaSTAGE, sfTable))
  ...
  try:
    df.write\
      ...
      .option("dbtable",  "{}.{}".format(schemaSTAGE, sfTable))\
      .mode(mode)\
      .save()
    print("{}.{} successfully loaded into Snowflake !".format(schemaSTAGE, sfTable))
  except:
    print("Error loading table {}.{} in Snowflake !".format(schemaSTAGE, sfTable))  
    raise
...
# sfCount  = curIDL.execute(query.format(pk,pk, schema, sfTable))
# cdmCount = sqlContext.sql(query.format(pk,pk, 'cdm_prod', cdm_table))
...
#   print('-CDM cdm_prod.{} has '.format(cdm_table) + str(cdmCount))
