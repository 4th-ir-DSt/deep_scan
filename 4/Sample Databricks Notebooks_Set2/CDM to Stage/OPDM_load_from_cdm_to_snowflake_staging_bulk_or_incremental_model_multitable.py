...
cdmtablelist = ["LOCATION_HIERARCHY_PLR", "STR_OPS_EMPLOYEEPAY_DAY_PLR", "STR_OPS_LABOR_QTRHR_PLR",
                "STR_OPS_SALESFORECAST_DAY_PLR", "STR_OPS_SALESFORECAST_QTRHR_PLR", "STR_OPS_SCHEDULEDANDACTUALHOURS_PLR",
                "STR_OPS_SPEEDOFSERVICE_PLR", "STR_OPS_STOREHOURS_PLR", "STR_OPS_VISITSURVEY_PLR",
                "TRAN_ORDERLINEPREP_PLR", "TRAN_EMPLOYEE_PLR"]
...
for tablename in cdmtablelist:
      ...
      if historyflag == 'True':
          ...
          curSTAGE.execute("""DELETE 
                            FROM {}.{}.{} 
                            WHERE CdmLoadDate BETWEEEN '{}' 
                            AND '{}' 
                            {}""".format(
          databaseSTAGE,schemaSTAGE, sfSTAGETable, historyloadstart, historyloadend, brand_and_condition))
      ...
      if historyflag == 'True':
         df = sqlContext.sql("""SELECT * 
                                FROM {}.{} 
                                WHERE CdmLoadDate BETWEEN '{}' 
                                AND '{}' 
                                {}""".format(
             databricks, cdm_table, historyloadstart, historyloadend, brand_and_condition))
        ...
        if delta == '*': # possible bug here, this operation is not idempotent and in case we run two times against the same set of data it will load two times the same set
          df = sqlContext.sql("""SELECT * 
                                 FROM {}.{} 
                                 {}""".format(databricks, cdm_table, brand_where_condition))
          ...
          loadDate = curSTAGE.execute("""SELECT IFNULL(MIN(cdmloaddate),'17530101') cdmloaddate 
                                         FROM 
                                             ...
                                             FROM  {}.{}.{} 
                                             {} 
                                             ORDER BY cdmloaddate DESC)""".format(
              load_numberofdaysback, databaseSTAGE, schemaSTAGE, sfSTAGETable, brand_where_condition)).fetchone()[0]
          curSTAGE.execute("""DELETE 
                              FROM  {}.{}.{} 
                              WHERE cdmloaddate>='{}' 
                              {}""".format(
              databaseSTAGE, schemaSTAGE, sfSTAGETable,loadDate, brand_and_condition))
          #df = sqlContext.sql("SELECT * FROM {}.{} WHERE CAST(CdmLoadDate AS INT) > '{}'".format(databricks,cdm_table, loadDate))
          print("delete data from table {}.{}.{} {} since date {}".format(
              databaseSTAGE, schemaSTAGE, sfSTAGETable, brand_where_condition, loadDate))
          print(datetime.now())
          df = sqlContext.sql("""SELECT * 
                                 FROM {}.{} 
                                 WHERE CdmLoadDate >= '{}' 
                                 {}""".format(
              databricks, cdm_table, loadDate, brand_and_condition))
