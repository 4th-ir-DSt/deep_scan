...
if historyflag=='True':
  ...
  if delta == '*':
    ...
    loadDate = curIDL.execute("select IFNULL(min(cdmloaddate),'17530101') cdmloaddate from (select distinct top {} cdmloaddate from {}.{}.{} order by cdmloaddate desc)".format(load_numberofdaysback,databaseSTAGE,schemaSTAGE, sfSTAGETable)).fetchone()[0]
