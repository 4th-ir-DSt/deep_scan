...
def load_ext(provider_name, retailer_name, retailer_dir_path, logger, spark):
    ...
    cdna_sat_raw_table                  = config_op_obj["cn_dim"]["CDNA_SAT_RAW_TABLE"]
    cdna_sat_raw_table_path             = config_op_obj["cn_dim"]["CDNA_SAT_RAW_TABLE_PATH"]
    cdna_sat_sourcepath                 = config_op_obj["cn_dim"]["CDNA_RAW_SAT_SOURCE_PATH"]
    cdna_sat_filename                   = config_op_obj["cn_dim"]["CDNA_RAW_SAT_FILENAME"]

    logger.info("\nParameter details are ")
    logger.info("=================================================================")
    logger.info('cdna_sat_raw_table      : '+cdna_sat_raw_table)
    logger.info('cdna_sat_raw_table_path : '+cdna_sat_raw_table_path)
    logger.info('cdna_sat_sourcepath     : '+cdna_sat_sourcepath)
    logger.info('cdna_sat_filename       : '+cdna_sat_filename)
    logger.info("=================================================================\n")


    logger.info("Source file availablity checking : ")
    logger.info("=================================\n")
    logger.info("Source file name is :{}".format(cdna_sat_filename))
    logger.info("Source file path is :{}\n".format(cdna_sat_sourcepath))



    fileNamePattern=str(cdna_sat_sourcepath)+str(cdna_sat_filename)+'*'
   ...
   #Cheking is article external raw table exists or not if not exists creating table
    if (spark.catalog._jcatalog.tableExists(cdna_sat_raw_table)== False):

      try:
         print('')
         print('Creating  external raw table: {}'.format(cdna_sat_raw_table))
         print('=============================================')
         print('')
         sql_to_run=("create external table {0} (dunnhumby_customerdnaattributes_hkey string,shabit string,pricesenstivity string,shoppingmission string,customerprofitability string,createdate string,origin_file_name string,run_id string,date_key string,load_date string) partitioned by (segment int)  stored as parquet location '{1}' ".format(cdna_sat_raw_table,cdna_sat_raw_table_path))
         print('')
         print(sql_to_run)
         print('')
         spark.sql(sql_to_run)
         print('Sucessfully created external table {}'.format(cdna_sat_raw_table))
         print('=============================================\n')
      except Exception as ex:
          logger.info("Error in creating customerdnaattributes SAT external table")
          logger.error('\n======================== ERROR ENCOUNTERED in %s ======================== os_error_exit')
          logger.error(str(ex))
          logger.error('================================= PROCESS TERMINATED ====================================')
          raise Exception("Error in creating customerdnaattributes SAT external table")
    ...
    for i in range(len(files)):
        ...
        segment_path = cdna_sat_raw_table_path + "segment=" + file_date[0].decode()+'/'
        ...
        if ("history" in file_name.lower()):
            ...
            print('')
            print("File Unzipping and Loading process Started...")
            print('')
            ...
            logger.info("This is delta File : %s " %(file_name))
            print('=============================================')
            hadoop_create_dir = "hadoop fs -mkdir -p " + segment_path
            print ("Creating segment folder if not exists in hadoop path :\n\nCommand is : %s " %(hadoop_create_dir))
            if (os.system(hadoop_create_dir) != 0):
            ...
            command = "hadoop fs -copyFromLocal {0}{1} {2}".format(cdna_sat_sourcepath, file_name, segment_path)
            logger.info("command is : %s " %(command))
            if (os.system(command) != 0):
                logger.error("Failed to run the command : %s" %(command))
    ...
    try:
        ...
        sql_query =("MSCK REPAIR TABLE {}".format(cdna_sat_raw_table))
        print('')
        print("Add new partitions into history table :")
        print("Command is  : "+ sql_query)
        spark.sql(sql_query)
        print('')
        print(" MSCK REPAIR TABLE {} Command Executed Sucessfully ".format(cdna_sat_raw_table))
    except Exception as ex:
...
#Main
##########################################################################################################################################################
# Description:      This is the main function for building the customerdnaattributes stage  table
...
def main(argument_list):
    ...
    logger.info('\n=====================================================')
    logger.info('SCRIPT NAME    : shoprite_customerdnaattributes_sat_raw_table_load.py')
