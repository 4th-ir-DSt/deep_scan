...
def load_ext(provider_name, retailer_name, retailer_dir_path, logger, spark):
    ...
    cn_sat_raw_table                  = config_op_obj["cn_dim"]["CN_SAT_RAW_TABLE"]
    cn_sat_raw_table_path             = config_op_obj["cn_dim"]["CN_SAT_RAW_TABLE_PATH"]
    cn_sat_sourcepath                 = config_op_obj["cn_dim"]["CN_RAW_SAT_SOURCE_PATH"]
    cn_sat_filename                   = config_op_obj["cn_dim"]["CN_RAW_SAT_FILENAME"]

    logger.info("\nParameter details are ")
    logger.info("=================================================================")
    logger.info('cn_sat_raw_table      : '+cn_sat_raw_table)
    logger.info('cn_sat_raw_table_path : '+cn_sat_raw_table_path)
    logger.info('cn_sat_sourcepath     : '+cn_sat_sourcepath)
    logger.info('cn_sat_filename       : '+cn_sat_filename)
    logger.info("=================================================================\n")


    logger.info("Source file availablity checking : ")
    logger.info("=================================\n")
    logger.info("Source file name is :{}".format(cn_sat_filename))
    logger.info("Source file path is :{}\n".format(cn_sat_sourcepath))


    
    fileNamePattern=str(cn_sat_sourcepath)+str(cn_sat_filename)+'*'
   ...
   #Cheking is article raw table exista or not if not exists creating table
    if (spark.catalog._jcatalog.tableExists(cn_sat_raw_table)== False):
	
      try:
         print('')
         print('Creating  raw table: {}'.format(cn_sat_raw_table))
         print('')
         sql_to_run=("create table {} (ciam_customerprofile_detail_hkey string,householdidentifier string,brandmemberstatus string,prefferedstore string ,customercreationdate   string,customerupdateddate   string,loyaltycardnumber string,cardcreatedate string,cardupdatedate string,personalidentificationnumberprefix  string,birthday string,email string,mobilenumberprefix string,registrationchannel string,countryofissue string,registrationagent string,run_id string,date_key string,load_date string) partitioned by (segment int)  stored as parquet location '{}' ".format(cn_sat_raw_table,cn_sat_raw_table_path))
         print('')
         print(sql_to_run)
         print('')
         spark.sql(sql_to_run)
         print('Sucessfully created table {}'.format(cn_sat_raw_table))      
      except Exception as ex:
          logger.info("Error in creating consumer sat table")
          logger.error('\n======================== ERROR ENCOUNTERED in %s ======================== os_error_exit')
          logger.error(str(ex))
          logger.error('================================= PROCESS TERMINATED ====================================')
          raise Exception("Error in creating consumer sat table")
    ...
    for i in range(len(files)):
        ...
        segment_path = cn_sat_raw_table_path + "segment=" + file_date[0].decode()+'/'
        ...
        if ("history" in file_name.lower()):
            ...
            if (os.system(command) != 0):
              ...
              print('')
              print('File {} successfully unzipped and Loaded into {} path'.format(latestfile,segment_path))
              print('')
            ...
            print('')
            command = "hadoop fs -copyFromLocal {0}{1} {2}".format(cn_sat_sourcepath, file_name, segment_path)
            logger.info("command is :%s " %(command))
            if (os.system(command) != 0):
                logger.error("Failed to run the command : %s" %(command))
    ...
    try:
        print('')        
        sql_query =("MSCK REPAIR TABLE {}".format(cn_sat_raw_table))
        print('')
        print("Add new partitions into history table :")
        print("Command is  : "+ sql_query)
        spark.sql(sql_query)
        print('')
        print("MSCK REPAIR TABLE {} Command Executed Sucessfully ".format(cn_sat_raw_table))
    except Exception as ex:
...
#Main
##########################################################################################################################################################
# Description:      This is the main function for building the sat consumer stage  table
...
def main(argument_list):
    ...
    logger.info('\n=====================================================')
    logger.info('SCRIPT NAME    : shoprite_consumer_sat_raw_table_load.py')
